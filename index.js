...
// ===========================================================================
// v584 [IMAP-NAO-VAZA] — rede de seguranca para as conexoes de e-mail.
//
// O CRM abre IMAP em sete lugares (painel de contratos assinados, leitura da
// caixa, anexos, zip de anexos...). Em seis deles o logout ficava fora do
// finally: bastava a varredura estourar no meio para a conexao ficar pendurada
// no servidor. O cPanel limita conexoes simultaneas por usuario, entao o login
// seguinte era recusado e o log mostrava 'authenticationFailed' — com a senha
// perfeitamente correta.
//
// Em vez de confiar em sete blocos lembrarem de fechar, o proprio connect()
// arma o fechamento. Nenhuma leitura nossa passa de quatro minutos; o que
// passar disso e conexao esquecida, e ela cai sozinha.
// ===========================================================================
try {
  const _ImapFlowCls = require('imapflow').ImapFlow;
  if (_ImapFlowCls && !_ImapFlowCls.prototype.__v584Guardiao) {
    const _connectOriginal = _ImapFlowCls.prototype.connect;
    _ImapFlowCls.prototype.connect = async function () {
      const r = await _connectOriginal.apply(this, arguments);
      const _corta = setTimeout(() => {
        try { console.warn('[IMAP-NAO-VAZA] conexao aberta ha 4 min sem fechar — encerrando.'); } catch (e) {}
        try { this.close(); } catch (e) {}
      }, 4 * 60 * 1000);
      if (_corta.unref) _corta.unref();     // um relogio pendente nao segura o processo
      const _desarma = () => { try { clearTimeout(_corta); } catch (e) {} };
      try { this.once('close', _desarma); this.once('error', _desarma); } catch (e) {}
      const _logoutOriginal = this.logout.bind(this);
      this.logout = async function () { _desarma(); return await _logoutOriginal(); };
      return r;
    };
    _ImapFlowCls.prototype.__v584Guardiao = true;
  }
} catch (e) { /* sem imapflow instalado: nada a proteger */ }
...
app.get('/api/leads', authenticateToken, async (req, res) => {
  try {
    ...
    // preenchido ou bridge=1 sao cross-visiveis entre pre/pos e NUNCA sao cortados.
    {
      ...
      // =====================================================================
      // v605 [COLUNA-ARQUIVO] — teto de quantidade em "Lead declinou/cancelado".
      //
      // O Henry trabalha nas TRES primeiras colunas (Novos Leads, Tratamento
      // inicial, Venda convertida). Essas passam inteiras por aqui. A de
      // declinados tinha 876 cards de 2.559 do board — um terco do peso da tela
      // para um arquivo que ninguem abre no dia a dia.
      //
      // O corte por TEMPO logo acima nao resolvia: ele desliga junto com o
      // "Ver todos", e o board e usado com o "Ver todos" ligado. Por quantidade
      // e previsivel — sempre os 150 mais recentes, independente de o pedido ser
      // normal ou completo.
      //
      // Nada some do banco: quem precisa do historico inteiro tem o "Ver todos",
      // a guia Leads e a busca, que continuam enxergando tudo.
      // =====================================================================
      if (!_isAllParam) {
        const TETO_DECLINADO = 150;
        const _declinados = parsedLeads.filter(l => l.stage === 'declinado' && !l.pos_stage && l.bridge !== 1);
        if (_declinados.length > TETO_DECLINADO) {
          _declinados.sort((a, b) => (_lastActivityMs(b) || 0) - (_lastActivityMs(a) || 0));
          const _mantidos = new Set(_declinados.slice(0, TETO_DECLINADO).map(l => l.id));
          const _antes = parsedLeads.length;
          parsedLeads = parsedLeads.filter(l => {
            if (l.stage !== 'declinado' || l.pos_stage || l.bridge === 1) return true;
            return _mantidos.has(l.id);
          });
          console.log('[board] declinados: ' + _declinados.length + ' -> ' + TETO_DECLINADO
            + ' (board de ' + _antes + ' para ' + parsedLeads.length + ' cards)');
        }
      }
    }
...
async function _r9Faltantes(cur) {
  // [DATA-VENDA] A data da venda NAO entra na lista de bloqueio, e de proposito: ela e gravada
  // automaticamente na transferencia (padrao = o dia de hoje, editavel depois no card) e isso
  // acontece DEPOIS desta validacao — exigi-la aqui barraria toda venda, inclusive as corretas.
  // O campo existe no card e pode ser corrigido a qualquer momento; quem manda no faturamento e ele.
...
app.patch('/api/leads/:id', authenticateToken, async (req, res) => {
  ...
  if (score !== undefined && score !== null && score !== '') {
    const _sc = Number(score);
    if (!Number.isInteger(_sc) || _sc < 1 || _sc > 10) return res.status(400).json({ error: "score deve ser um inteiro de 1 a 10" });
  }
  if (status_pagamento !== undefined && !['lead', 'taxa_paga', 'contrato_ativo'].includes(String(status_pagamento))) {
    return res.status(400).json({ error: "status_pagamento inválido (use: lead, taxa_paga ou contrato_ativo)" });
  }
  ...
  try {
    ...
    // execucao6 (2026-07-25): vínculo card ↔ contrato (botão 📄 Contrato do modal; '' limpa).
    if (contract_id !== undefined) {
      updates.push("contract_id = ?");
      params.push(String(contract_id || '').trim() || null);
    }
    ...
    // execucao1 Sprint1 (2026-07-25): qualificação D-P-Q + score + pagamento + kill-switch da IA.
    if (destino_visto !== undefined) { updates.push("destino_visto = ?"); params.push(String(destino_visto || '')); }
    if (tipo_visto !== undefined) { updates.push("tipo_visto = ?"); params.push(String(tipo_visto || '')); }
    if (prazo_viagem !== undefined) { updates.push("prazo_viagem = ?"); params.push(String(prazo_viagem || '')); }
    if (qtd_solicitantes !== undefined) { updates.push("qtd_solicitantes = ?"); params.push((qtd_solicitantes === null || qtd_solicitantes === '') ? null : (Number(qtd_solicitantes) || null)); }
    if (score !== undefined) { updates.push("score = ?"); params.push((score === null || score === '') ? null : Number(score)); }
    if (status_pagamento !== undefined) { updates.push("status_pagamento = ?"); params.push(String(status_pagamento)); }
    if (bot_ativo !== undefined) { updates.push("bot_ativo = ?"); params.push((bot_ativo === 0 || bot_ativo === '0' || bot_ativo === false) ? 0 : 1); }
    ...
    // [FU-TIRA-DE-NOVOS] (caso Marli, 2026-08-10) Marcar follow-up num card que ainda esta em
    // "Novos Leads" nao o tirava de la: ficava com a tarja de follow-up numa coluna onde a fila de
    // follow-up nem existe, invisivel para quem trabalha o Tratamento. Agora, ao receber data de
    // follow-up, o card sai de "Novos Leads" e vai para o Tratamento inicial — e la a subcoluna e
    // decidida pela regra normal: com o cliente esperando resposta vai para "Lead respondendo",
    // sem ninguem esperando vai para "Follow-up manual".
    // Card que JA esta no Tratamento nao muda de coluna: isso o Henry mandou desfazer em 08/08
    // ("se ele mandou mensagem deve ir para lead respondendo"), e continua valendo.
    try {
      const _temFu = (followup_date !== undefined && followup_date) || (priority !== undefined && priority === 'followup');
      if (_temFu) {
        const _cur = await getRow("SELECT stage, name, phone FROM leads WHERE id = ?", [id]);
        if (_cur && _cur.stage === 'novo') {
          await runQuery("UPDATE leads SET stage = 'tratamento', coluna_origem = COALESCE(NULLIF(coluna_origem,''), 'novo') WHERE id = ? AND stage = 'novo'", [id]);
          try { logLeadHistory({ leadId: id, phone: _cur.phone, name: _cur.name, type: 'movimentacao', detail: 'Follow-up marcado — card saiu de "Novos Leads" para o Tratamento inicial', meta: { from: 'novo', to: 'tratamento', fu: 1 } }); } catch (e) {}
          console.log('[FU-TIRA-DE-NOVOS] "' + (_cur.name || id) + '": follow-up marcado, card movido de Novos Leads para o Tratamento.');
        }
      }
    } catch (e) { console.error('[FU-TIRA-DE-NOVOS]', e && e.message); }
...
app.get('/api/dashboard', authenticateToken, async (req, res) => {
  try {
    ...
    const _byDay = {};
    _range.forEach(d => { _byDay[d.iso] = { day: d.label, ga: 0, meta: 0, org: 0, semclass: 0, total: 0, origins: {} }; });
    const _allInRange = await allRows(
      "SELECT createdAt, tracking, source, source_locked, tags FROM leads WHERE substr(createdAt,1,10) >= ? AND substr(createdAt,1,10) <= ? AND archived = 0 AND COALESCE(source,'') <> 'Planilha Americano'",
      [_range[0].iso, _range[_range.length - 1].iso]
    );
    // execucao7: origem DINÂMICA (lista configurada em Configurações → Origens de lead) por dia,
    // em paralelo aos 4 baldes antigos (retrocompat) — o front novo lê slot.origins.
    const _originsCfg = await getLeadOriginsList();
    _allInRange.forEach(l => {
      ...
      slot[cat]++; slot.total++;
      try { const lb = leadOriginLabel(l, _originsCfg); slot.origins[lb] = (slot.origins[lb] || 0) + 1; } catch (e) {}
    });
...
app.get('/api/dashboard/channel-leads', authenticateToken, async (req, res) => {
  try {
    ...
    // execucao7: aceita ALTERNATIVAMENTE origin=<rótulo> (origem dinâmica da lista configurada) —
    // usado pelo gráfico novo; channel=ga|meta|org|semclass continua p/ retrocompat.
    const origin = String(req.query.origin || '').trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || (!origin && ['ga', 'meta', 'org', 'semclass'].indexOf(channel) < 0)) {
      return res.status(400).json({ error: 'Parâmetros: date=YYYY-MM-DD e channel=ga|meta|org|semclass (ou origin=<rótulo>)' });
    }
    ...
    const _originsCfg = origin ? await getLeadOriginsList() : null;
    const leads = rows
      .filter(l => origin ? (leadOriginLabel(l, _originsCfg) === origin) : (leadChannelCat(l) === channel))
  ...
  try {
    ...
    try {
      ...
      if (acc && ImapFlow) {
        ...
        // v584: daqui para baixo, fecha a conexao aconteca o que acontecer.
        try {
        ...
        try {
          ...
          try { await client.logout(); } catch (e) { try { client.close(); } catch (e2) {} }
        }
      }
    } catch (err) {
...
app.get('/api/conversations', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // v607 [LISTA-ENXUTA]: a ultima mensagem vai RESUMIDA.
    //
    // Medido: lastMessage era 337 KB dos 682 KB da resposta — metade do peso — e a
    // guia pede esta rota nove vezes num carregamento. A lista mostra uma linha
    // cortada com reticencias, mas vinha o objeto inteiro: texto completo (um audio
    // transcrito passa de mil caracteres), caminho de midia, status, tipo.
    //
    // Os quatro campos abaixo sao os que a tela usa. Quem ABRE a conversa recebe tudo
    // pela rota das mensagens — isto aqui e so o resumo da lista.
    const _RESUMO_MSG = 140;
    const _resumeMsg = (m) => {
      if (!m) return null;
      let t = m.text == null ? '' : String(m.text);
      if (t.length > _RESUMO_MSG) t = t.slice(0, _RESUMO_MSG) + '…';
      return { id: m.id, from: m.from, text: t, timestamp: m.timestamp, type: m.type };
    };
    const detailedConvs = convs.map(c => ({
      ...c,
      online: Boolean(c.online),
      lastMessage: _resumeMsg(lastMsgMap.get(c.id))
    }));
...
app.get('/api/conversations/:id', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const messages = await allRows(
      "SELECT id, \`from\`, text, time, type, timestamp, status, edited, deleted, our_number, quotedId, quotedText, quotedFrom, transcription FROM messages WHERE conversationId = ? ORDER BY timestamp ASC",
      [id]
    );
...
// v585: a logica de reenvio virou funcao para ter UM dono so. O botao do menu, o
// botao do painel e o "Reenviar todos" passam todos por aqui — assim nenhuma regra
// (a trava de uma tentativa, a linha dona, o antiban) fica valendo em um caminho e
// esquecida no outro. Devolve { erro } em vez de responder: quem chama e que decide
// se isso vira HTTP 409 ou uma linha no relatorio do lote.
async function _reenviarUma(msgId, opcoes) {
  const _op = opcoes || {};
  ...
  if (!m || Number(m.deleted || 0) === 1 || Number(m.status || 0) >= 3) {
    ...
    return { erro: 'Mensagem ja confirmada ou inexistente', jaEntregue: true };
  }
  const convo = await getRow("SELECT id, account, jid_account, whatsapp_jid FROM conversations WHERE id = ?", [m.conversationId]);
  if (!convo) return { erro: 'Conversa nao existe mais' };

  // v584 [REENVIO-UMA-VEZ]: uma tentativa por mensagem, venha o pedido de onde vier.
  // Caso Roberto Ferri (12/08): quatro copias da mesma frase, porque cada reenvio
  // gerava uma mensagem nova que tambem ficava sem recibo e voltava para a fila de
  // reenvio 3h depois.
  const _trava = await deliveryWatch.motivoParaNaoReenviar(msgId);
  if (_trava) return { erro: _trava, travado: true };

  // Linha dona: jid_account primeiro; se nao houver, mapeia messages.our_number para a conta.
  ...
  if (!linha) return { erro: 'Nao ha linha dona para esta conversa' };

  // No lote, o governador anti-banimento manda: cap por hora e por dia do numero,
  // e intervalo com jitter entre um envio e o proximo DA MESMA LINHA. No clique
  // avulso nao entra — uma mensagem so nao e rajada, e travar o operador seria pior.
  if (_op.antiban) {
    const pode = await antiban.canSendProactive(linha);
    if (!pode.ok) return { erro: pode.reason, capAtingido: true, linha };
    await antiban.pace(linha);
  }
  ...
  if (tipo !== 'text') {
    ...
    if (!mp || !fs.existsSync(mp)) return { erro: 'Arquivo de midia nao esta mais no servidor' };
  ...
  // Linha fora do ar nao recusa (regra v508): entra na fila e sai sozinha quando ela voltar.
  ...
  if (_op.antiban) { try { await antiban.recordSend(linha); } catch (e) {} }
  // A mensagem nova entra pelo caminho normal de envio, portanto o vigia ja a arma sozinho.
  await deliveryWatch.dismiss(msgId, 'reenvio');
  // v584: guarda o elo origem -> copia. E o que impede a copia da copia.
  try { await deliveryWatch.registraReenvio(msgId, (novo && novo.id) || null, m.conversationId); } catch (e) {}
  bustConversationsCache();
  console.log('[resend] ' + msgId + ' (' + tipo + ') reenviado pela linha ' + linha + (aberta ? '' : ' (na fila)'));
  return { ok: true, novoId: (novo && novo.id) || null, linha, fila: !aberta };
}

app.post('/api/delivery-alerts/:msgId/resend', authenticateToken, async (req, res) => {
  try {
    const r = await _reenviarUma(req.params.msgId, { antiban: false });
    const count = await deliveryWatch.countOpen();
    if (r.erro) return res.status(409).json({ error: r.erro, travado: !!r.travado, count });
    res.json({ ok: true, novoId: r.novoId, linha: r.linha, fila: r.fila, count });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ===========================================================================
// v585 [REENVIAR-TODOS] — o lote que o Henry dispara, nao o que rodava sozinho.
//
// Existia antes um processo no pm2 reenviando sozinho, e era ele que fazia o
// cliente receber a mesma frase quatro vezes. Aquilo foi desligado. Este aqui e
// o oposto: so anda quando alguem clica, manda uma por vez com intervalo de
// 20-60s por linha, respeita o cap do numero e para no meio se for pedido.
//
// O progresso vive em memoria de proposito: se o pm2 reiniciar no meio, o lote
// morre junto — e melhor parar do que retomar sozinho um envio que ninguem esta
// mais olhando.
// ===========================================================================
let _loteReenvio = { rodando: false, total: 0, feitos: 0, enviados: 0, pulados: 0, erros: 0,
                     parar: false, iniciadoEm: 0, atual: '', linhas: [] };

function _loteResumo() {
  const r = _loteReenvio;
  const faltam = Math.max(0, r.total - r.feitos);
  return { rodando: r.rodando, total: r.total, feitos: r.feitos, enviados: r.enviados,
           pulados: r.pulados, erros: r.erros, atual: r.atual, faltam,
           // 40s e a media do intervalo (20-60s) — serve para a tela dizer quanto falta.
           segundosRestantes: faltam * 40, iniciadoEm: r.iniciadoEm,
           linhas: r.linhas.slice(-40) };
}

async function _rodaLoteReenvio(lista) {
  const r = _loteReenvio;
  try {
    for (const item of lista) {
      if (r.parar) { r.linhas.push('— interrompido por voce —'); break; }
      r.atual = item.nome || item.msg_id;
      let res;
      try { res = await _reenviarUma(item.msg_id, { antiban: true }); }
      catch (e) { res = { erro: String(e && e.message || e) }; }
      r.feitos++;
      if (res && res.ok) {
        r.enviados++;
        r.linhas.push('✓ ' + r.atual + (res.fila ? ' (na fila: a linha esta fora do ar)' : ''));
      } else if (res && (res.travado || res.jaEntregue || res.capAtingido)) {
        r.pulados++;
        r.linhas.push('• ' + r.atual + ' — ' + res.erro);
      } else {
        r.erros++;
        r.linhas.push('✗ ' + r.atual + ' — ' + ((res && res.erro) || 'falha'));
      }
    }
  } finally {
    r.rodando = false; r.atual = '';
    console.log('[resend-lote] fim: ' + r.enviados + ' enviada(s), ' + r.pulados + ' pulada(s), ' + r.erros + ' com erro.');
  }
}

app.post('/api/delivery-alerts/resend-all', authenticateToken, async (req, res) => {
  try {
    if (_loteReenvio.rodando) return res.status(409).json({ error: 'Ja existe um reenvio em andamento.', ..._loteResumo() });
    const abertos = await deliveryWatch.listOpen(500);
    const lista = (abertos || []).map(a => ({ msg_id: a.msg_id, nome: a.conv_name || a.conv_phone || a.msg_id }));
    if (!lista.length) return res.status(409).json({ error: 'Nao ha nada para reenviar.' });
    _loteReenvio = { rodando: true, total: lista.length, feitos: 0, enviados: 0, pulados: 0, erros: 0,
                     parar: false, iniciadoEm: Date.now(), atual: '', linhas: [] };
    _rodaLoteReenvio(lista).catch(e => console.error('[resend-lote]', e && e.message));
    console.log('[resend-lote] inicio: ' + lista.length + ' mensagem(ns) na fila.');
    res.json({ ok: true, ..._loteResumo(), foraDoHorario: !antiban.isBusinessHours() });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

app.get('/api/delivery-alerts/resend-all/status', authenticateToken, async (req, res) => {
  try { res.json({ ..._loteResumo(), count: await deliveryWatch.countOpen() }); }
  catch (e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/delivery-alerts/resend-all/stop', authenticateToken, async (req, res) => {
  _loteReenvio.parar = true;
  res.json({ ok: true, ..._loteResumo() });
});
...
app.post('/api/conversations/:id/messages', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      // [CANAL-CERTO v611] conversa do Instagram nao entra no roteamento por linha de
      // WhatsApp. Era aqui que o account virava wa3/wa7 e o desvio do Direct (logo abaixo)
      // deixava de ser acionado — a resposta saia por WhatsApp para um endereco inexistente.
      const _ehIg = String((convo && convo.whatsapp_jid) || '').indexOf('ig:') === 0 || convo.account === 'ig';
      ...
      if (_ehIg || _ownLock || lineOpen) { /* Instagram, ou linha da conversa conectada — ela envia */ }
    ...
    if (convo.account === 'ig' || String(convo.whatsapp_jid || '').indexOf('ig:') === 0) {
      // [CANAL-CERTO v611] chave = endereco (ig:...), nao a conta. whatsapp_jid nunca muda;
      // account ja foi trocado por rotina automatica e foi assim que o Direct saiu do ar.
...
app.post('/api/conversations/:id/audio', authenticateToken, uploadSingle('file'), async (req, res) => {
  ...
  try {
    ...
    if (isConnected) {
      ...
      await runQuery("UPDATE conversations SET lastTime = ? WHERE id = ?", [timeStr, id]);
      // transcricao-20260728: transcreve o áudio salvo offline em segundo plano (fire-and-forget).
      fireTranscricaoAudio(msgId, mediaPath);
...
// 9b2. TRANSCRIÇÃO ON-DEMAND (transcricao-20260728): transcreve um áudio ANTIGO (gravado antes
// da transcrição automática existir, ou cuja transcrição falhou — transcription NULL). O
// frontend mostra o botão "📝 Transcrever" nesses casos e chama esta rota. Diferente dos hooks
// fire-and-forget, aqui o await é proposital: o usuário está esperando o texto na tela.
app.post('/api/messages/:msgId/transcribe', authenticateToken, async (req, res) => {
  try {
    const msg = await getRow("SELECT id, type, mediaPath, transcription FROM messages WHERE id = ?", [req.params.msgId]);
    if (!msg || msg.type !== 'audio' || !msg.mediaPath) {
      return res.status(404).json({ error: "Mensagem de áudio não encontrada" });
    }
    if (msg.transcription !== null && msg.transcription !== undefined) {
      return res.json({ transcription: msg.transcription }); // já transcrita — devolve sem custo
    }
    const { transcreverAudio } = require('./ai');
    const t = await transcreverAudio(msg.mediaPath);
    if (t === null || t === undefined) {
      return res.status(502).json({ error: "Falha na transcrição" });
    }
    await runQuery("UPDATE messages SET transcription = ? WHERE id = ?", [t, msg.id]);
    bustConversationsCache(); // messages mudou — derruba o micro-cache do GET /api/conversations
    res.json({ transcription: t });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
// [PERF-AVATAR] Esta rota era o maior gargalo do CRM: 212 requisicoes por carregamento do board,
// somando 353 SEGUNDOS de rede. O motivo: /opt/crm-backend/avatars esta VAZIO (0 arquivos para
// 2173 conversas), entao toda requisicao ia ao WhatsApp buscar a foto (segundos), falhava, e
// devolvia 404 SEM cache — repetindo o desperdicio a cada carregamento. Como o navegador so abre
// ~6 conexoes por dominio, essa fila atrasava TODAS as chamadas de API atras dela.
// Agora: a resposta NUNCA espera pelo WhatsApp. Sem arquivo = 404 na hora, com cache. A busca vai
// para uma fila em segundo plano, no maximo 2 por vez e 1 tentativa por contato a cada 6 horas.
const _AV_TTL_MS = 6 * 3600 * 1000;
const _avSemFoto = new Map();     // jid -> quando foi marcado como "nao tem foto"
const _avTentativa = new Map();   // jid -> ultima tentativa de buscar no WhatsApp
const _avTelJid = new Map();      // telefone -> { jid, t } (a consulta por telefone varre a tabela)
const _avFila = [];
let _avRodando = 0;
function _avAgenda(jid) {
  const ult = _avTentativa.get(jid) || 0;
  if (Date.now() - ult < _AV_TTL_MS) return;      // ja tentamos ha pouco
  if (_avFila.indexOf(jid) !== -1) return;
  if (_avFila.length > 200) return;               // teto de seguranca
  _avFila.push(jid);
  _avPuxa();
}
function _avPuxa() {
  while (_avRodando < 2 && _avFila.length) {
    const jid = _avFila.shift();
    _avTentativa.set(jid, Date.now());
    _avRodando++;
    (async () => {
      try {
        const sock = Object.values(sessions || {}).find(s => s);
        if (sock) await fetchAndStoreAvatar(sock, jid);
        if (fs.existsSync(avatarFileForJid(jid))) _avSemFoto.delete(jid);
      } catch (e) {}
      finally { _avRodando--; setTimeout(_avPuxa, 250); }
    })();
  }
}
app.get('/api/avatar', async (req, res) => {
  ...
  const _semFoto = (jid) => {
    _avSemFoto.set(jid || '?', Date.now());
    if (jid) _avAgenda(jid);
  ...
  try {
    ...
    if (!jid && phone.length >= 8) {
      const chave = phone.slice(-8);
      const cache = _avTelJid.get(chave);
      if (cache && (Date.now() - cache.t) < 3600000) {
        jid = cache.jid || '';
        if (!jid) { res.setHeader('Cache-Control', 'public, max-age=21600'); return res.status(404).end(); }
      } else {
        const row = await getRow("SELECT whatsapp_jid FROM conversations WHERE whatsapp_jid IS NOT NULL AND REPLACE(REPLACE(REPLACE(REPLACE(phone,'+',''),' ',''),'-',''),'(','') LIKE ? LIMIT 1", [`%${chave}%`]);
        jid = (row && row.whatsapp_jid) || '';
        _avTelJid.set(chave, { jid, t: Date.now() });
      }
    }
    if (!jid) { res.setHeader('Cache-Control', 'public, max-age=21600'); return res.status(404).end(); }

    // Ja sabemos que este contato nao tem foto: responde na hora, sem tocar em disco.
    const marca = _avSemFoto.get(jid);
    if (marca && (Date.now() - marca) < _AV_TTL_MS) {
      res.setHeader('Cache-Control', 'public, max-age=21600');
      return res.status(404).end();
    }

    // Instagram: a "foto" e o profile_pic da Graph API guardado em conversations.avatar.
    ...
    if (!st) return _semFoto(jid);                 // 404 IMEDIATO — a busca fica para o segundo plano

    // ETag pelo mtime+tamanho: o navegador que ja tem a foto recebe 304 e nao baixa de novo.
    const etag = '"av-' + st.size + '-' + Math.floor(st.mtimeMs) + '"';
...
// [RT-COMERCIAL] Horario comercial: quanto tempo entre dois instantes cai DENTRO do expediente.
// Motivo: o tempo de resposta nao pode punir a equipe pelo que o cliente escreveu de madrugada.
// Cliente escreve 22:00 de terca e a resposta sai 09:10 de quarta = 10 minutos, nao 11 horas.
// A configuracao e a MESMA da tela de Configuracoes (app_settings.business_hours) — mudou la,
// muda aqui, sem numero fixo no codigo. Fuso America/Sao_Paulo (UTC-3, sem horario de verao no
// Brasil desde 2019). Dia com on=false e data em holidays ('DD/MM', qualquer ano) somam ZERO.
const _BH_TZ_OFFSET_MS = -3 * 3600 * 1000;
const _BH_DIAS = ['sun', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat'];
let _bhCache = { t: 0, v: null };
async function carregaBH() {
  if (_bhCache.v && (Date.now() - _bhCache.t) < 60000) return _bhCache.v;
  let v = null;
  try {
    const row = await getRow("SELECT value FROM app_settings WHERE key = 'business_hours'");
    if (row && row.value) v = JSON.parse(row.value);
  } catch (e) { v = null; }
  _bhCache = { t: Date.now(), v: v || {} };
  return _bhCache.v;
}
// Converte um instante UTC para os campos do calendario LOCAL (Sao Paulo).
function _bhLocal(ms) {
  const d = new Date(ms + _BH_TZ_OFFSET_MS);
  return { dow: d.getUTCDay(), dia: d.getUTCDate(), mes: d.getUTCMonth() + 1,
    inicioDiaUTC: Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()) - _BH_TZ_OFFSET_MS };
}
function _bhHoraMs(hhmm, padrao) {
  const m = /^(\d{1,2}):(\d{2})$/.exec(String(hhmm || '').trim());
  if (!m) return padrao;
  return (Number(m[1]) * 60 + Number(m[2])) * 60000;
}
// Janela util [abre, fecha] em ms ABSOLUTOS do dia que contem 'ms'. null = dia sem expediente.
function _bhJanelaDoDia(ms, bh) {
  const L = _bhLocal(ms);
  const feriados = Array.isArray(bh && bh.holidays) ? bh.holidays : [];
  const chave = String(L.dia).padStart(2, '0') + '/' + String(L.mes).padStart(2, '0');
  if (feriados.some(h => String((h && h.date) || '').trim() === chave)) return null;
  const cfg = (bh && bh.days && bh.days[_BH_DIAS[L.dow]]) || null;
  if (!cfg || cfg.on === false) return null;
  const abre = L.inicioDiaUTC + _bhHoraMs(cfg.open, 9 * 3600000);
  const fecha = L.inicioDiaUTC + _bhHoraMs(cfg.close, 18 * 3600000);
  if (!(fecha > abre)) return null;
  return { abre, fecha };
}
// Milissegundos UTEIS entre dois instantes. Varre dia a dia (teto de 400 dias, para nunca virar
// laco infinito com datas corrompidas).
function msUteis(iniMs, fimMs, bh) {
  let ini = Number(iniMs) || 0, fim = Number(fimMs) || 0;
  if (!(fim > ini)) return 0;
  let total = 0, cursor = ini, guarda = 0;
  while (cursor < fim && guarda++ < 400) {
    const L = _bhLocal(cursor);
    const j = _bhJanelaDoDia(cursor, bh);
    if (j) {
      const a = Math.max(cursor, j.abre);
      const b = Math.min(fim, j.fecha);
      if (b > a) total += (b - a);
    }
    cursor = L.inicioDiaUTC + 86400000; // 00:00 local do dia seguinte
  }
  return total;
}
// Instante em que o expediente de HOJE comecou (ou, se hoje nao tem expediente/ainda nao abriu,
// a abertura do ultimo dia util anterior). Usado pelo indicador do cabecalho (escopo=hoje).
function inicioExpedienteHoje(agoraMs, bh) {
  let cursor = agoraMs;
  for (let i = 0; i < 14; i++) {
    const j = _bhJanelaDoDia(cursor, bh);
    if (j && j.abre <= agoraMs) return j.abre;
    cursor = _bhLocal(cursor).inicioDiaUTC - 1; // volta para o dia anterior
  }
  return agoraMs - 24 * 3600000;
}
...
app.post('/api/email/send', authenticateToken, async (req, res) => {
  const { to: _toRaw, cc: _ccRaw, subject, text, html, attachments } = req.body;
  // [EMAIL-MULTI] O campo "Para" com DOIS enderecos falhava ("Falha ao enviar e-mail") enquanto o
  // mesmo endereco em copia funcionava. Motivo: o formulario separa com PONTO-E-VIRGULA (padrao do
  // Outlook) e o servico de envio so entende VIRGULA — com ';' ele lia tudo como um endereco unico,
  // invalido. Aqui os enderecos sao normalizados antes de sair: ';' vira ',', espaços sobrando
  // caem, e o ponto final que passa junto quando se copia e cola ("...@hotmail.com.") e removido.
  const _normEnderecos = (v) => String(v || '')
    .split(/[;,]/)
    .map(x => x.trim().replace(/^[.\s]+|[.\s]+$/g, ''))
    .filter(Boolean)
    .join(', ');
  const to = _normEnderecos(_toRaw);
  const cc = _normEnderecos(_ccRaw);
  if (!to || !subject) return res.status(400).json({ error: "Destinatario e assunto sao obrigatorios" });
  {
    const _invalidos = to.split(', ').concat(cc ? cc.split(', ') : []).filter(e => !/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(e));
    if (_invalidos.length) return res.status(400).json({ error: 'Endereço inválido: ' + _invalidos.join(', ') });
  }
...
// execucao8 (pedido do Henry, 2026-07-26): Configuração dos Cards — visibilidade por parâmetro,
// por coluna do pipeline (pré e pós) e por posição (fora/dentro do card). JSON livre no formato
// { "<pre|pos>:<stageId>": { ext: {param:0}, int: {param:0} } } — só os OCULTOS são gravados
// (ausente = visível). Quem aplica é o inject (crmApplyCardVis / crmModalApplyVis).
app.get('/api/settings/card-visibility', authenticateToken, async (req, res) => {
  try {
    const row = await getRow("SELECT value FROM app_settings WHERE key = 'card_visibility'");
    let cfg = {};
    try { cfg = row && row.value ? JSON.parse(row.value) : {}; } catch (e) { cfg = {}; }
    if (!cfg || typeof cfg !== 'object' || Array.isArray(cfg)) cfg = {};
    res.json(cfg);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/settings/card-visibility', authenticateToken, async (req, res) => {
  if (req.user && req.user.role === 'Vendedor') {
    return res.status(403).json({ detail: "Sem permissão para alterar configurações" });
  }
  try {
    const cfg = (req.body && typeof req.body === 'object' && !Array.isArray(req.body)) ? req.body : {};
    await runQuery(
      "INSERT INTO app_settings (key, value) VALUES ('card_visibility', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value",
      [JSON.stringify(cfg)]
    );
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
...
// execucao8: contratos MANUAIS locais (tabela manual_contracts) — mesclados à lista do PHP.
async function manualContractsList() {
  try {
    const rows = await allRows("SELECT * FROM manual_contracts ORDER BY created_at DESC");
    return (rows || []).map(m => ({
      id: m.id,
      manual: 1,
      closedNumber: null,
...
app.get('/api/contracts', authenticateToken, async (req, res) => {
  // cache de 30s (evita re-login a cada abertura/poll) — o cache guarda SÓ a lista do PHP;
  // os manuais locais são lidos frescos a cada request (baratos, SQLite local).
  const manual = await manualContractsList();
  if (_contractsCache.data && (Date.now() - _contractsCache.ts < 30 * 1000)) {
    return res.json(manual.concat(_contractsCache.data));
  }
  try {
    const token = await ds160AdminToken();
    ...
    res.json(manual.concat(list));
  } catch (e) {
    // em falha, devolve o último resultado bom se houver (sempre com os manuais na frente)
    if (_contractsCache.data) return res.json(manual.concat(_contractsCache.data));
    res.status(502).json({ error: e.message });
  }
});

// execucao8: criar contrato MANUAL (opção "não enviar para assinatura") — grava só no CRM.
// DEFINIDO ANTES de /api/contracts/:id para não ser sombreado pela rota de detalhe do PHP.
app.post('/api/contracts/manual', authenticateToken, async (req, res) => {
  if (req.user && req.user.role === 'Vendedor') {
    return res.status(403).json({ detail: 'Sem permissão para criar contratos' });
  }
  try {
    const b = req.body || {};
    const name = String(b.clientName || '').trim();
    if (!name) return res.status(400).json({ error: 'Informe o nome do cliente' });
    const id = 'M-' + require('crypto').randomBytes(4).toString('hex').toUpperCase();
    const docs = Array.isArray(b.docs) ? b.docs.slice(0, 30) : [];
    await runQuery(
      "INSERT INTO manual_contracts (id, client_name, client_email, client_phone, client_cpf, title, contract_text, payment_terms, price, docs, send_flag, created_at, address, applicants, visa_type, email2, referral) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
      [id, name, String(b.clientEmail || '').trim(), String(b.clientPhone || '').trim(),
       String(b.clientCpf || '').trim(), String(b.title || '').trim(), String(b.contractText || ''),
       String(b.paymentTerms || ''), String(b.price || '').trim(), JSON.stringify(docs), 0,
       new Date().toISOString(),
       String(b.address || '').trim(), String(b.applicants || '').trim(),
       String(b.visaType || '').trim(), String(b.email2 || '').trim(), String(b.referral || '').trim()]
    );
    res.json({ success: true, id });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
// Detalhe de um contrato manual (a rota /:id do PHP não o conhece).
app.get('/api/contracts/manual/:id', authenticateToken, async (req, res) => {
  try {
    const m = await getRow("SELECT * FROM manual_contracts WHERE id = ?", [String(req.params.id || '')]);
    if (!m) return res.status(404).json({ error: 'Contrato manual não encontrado' });
    let docs = []; try { docs = JSON.parse(m.docs || '[]'); } catch (e) {}
    res.json({
      id: m.id, manual: 1, clientName: m.client_name || '', clientEmail: m.client_email || '',
      clientPhone: m.client_phone || '', clientCpf: m.client_cpf || '', title: m.title || '',
      contractText: m.contract_text || '', paymentTerms: m.payment_terms || '', price: m.price || '',
      docs, createdAt: m.created_at || null,
      address: m.address || '', applicants: m.applicants || '', visaType: m.visa_type || '',
      email2: m.email2 || '', referral: m.referral || ''
    });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
// ===== execucao5 (2026-07-25): anexos de documentos da proposta + assinatura padrão da Vale Visto.
// ATENÇÃO À ORDEM: /signature (path fixo) precisa vir ANTES do GET /api/contracts/:id abaixo.
const CONTRACT_DOCS_DIR = path.join(__dirname, 'contract_docs');
try { fs.mkdirSync(CONTRACT_DOCS_DIR, { recursive: true }); } catch (e) {}
app.post('/api/contracts/docs', authenticateToken, upload.array('files', 10), async (req, res) => {
  if (req.user && req.user.role === 'Vendedor') return res.status(403).json({ error: 'Sem permissão' });
  try {
    const out = [];
    for (const f of (req.files || [])) {
      const safe = Date.now().toString(36) + '-' + Math.random().toString(36).slice(2, 8) + '-' + String(f.originalname || 'doc').replace(/[^\w.\- ]/g, '_').slice(0, 120);
      fs.writeFileSync(path.join(CONTRACT_DOCS_DIR, safe), f.buffer);
      out.push({ name: f.originalname || safe, file: safe, url: '/api/contracts/docs/' + encodeURIComponent(safe) });
    }
    res.json({ files: out });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.get('/api/contracts/docs/:file', authenticateToken, (req, res) => {
  const safe = String(req.params.file || '').replace(/[\\/]/g, '');
  const fp = path.join(CONTRACT_DOCS_DIR, safe);
  if (!fs.existsSync(fp)) return res.status(404).json({ error: 'Arquivo não encontrado.' });
  res.sendFile(fp);
});
app.get('/api/contracts/signature', authenticateToken, async (req, res) => {
  try {
    const row = await getRow("SELECT value FROM app_settings WHERE key = 'contract_admin_signature'");
    res.json({ signature: (row && row.value) || null });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/contracts/signature', authenticateToken, async (req, res) => {
  if (req.user && req.user.role === 'Vendedor') return res.status(403).json({ error: 'Sem permissão' });
  const sig = String((req.body && req.body.signature) || '');
  if (sig.indexOf('data:image/') !== 0 || sig.length > 500000) return res.status(400).json({ error: 'Assinatura inválida.' });
  try {
    await runQuery("INSERT INTO app_settings (key, value) VALUES ('contract_admin_signature', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value", [sig]);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post('/api/contracts', authenticateToken, async (req, res) => {
  ...
  // execucao5 (2026-07-25): dados extras do cliente + anexos — vão dentro do servicesScope
  // (blob livre que o contracts.php guarda como veio; nenhuma mudança no PHP).
  const clientPhone = String(b.clientPhone || '').trim();
  const clientCpf = String(b.clientCpf || '').trim();
  const contractDocs = Array.isArray(b.docs) ? b.docs.slice(0, 10) : [];
  ...
  try {
    ...
    const cr = await fetch(DS160_BASE + '/contracts.php?action=create', {
      ...
      body: JSON.stringify({
        ...
        servicesScope: { applicantsRange, basePrice, optionalServices, customClauses, clientPhone, clientCpf, docs: contractDocs }
      })
    });
    const cj = await cr.json().catch(() => null);
...
// ===== execucao1 Sprint1 (2026-07-25): COMPROMISSOS =====
// "Te ligo sábado às 11h" vira registro cobrável: callback / follow-up / retomada agendada.
const COMPROMISSO_TIPOS = ['callback', 'followup', 'retomada'];
...
// Lista os compromissos de um lead.
app.get('/api/leads/:id/compromissos', authenticateToken, async (req, res) => {
  try {
    const rows = await allRows("SELECT * FROM compromissos WHERE lead_id = ? ORDER BY data_hora ASC", [req.params.id]);
    res.json(rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// Cria um compromisso para o lead.
app.post('/api/leads/:id/compromissos', authenticateToken, async (req, res) => {
  const { tipo, data_hora, responsavel, notas } = req.body || {};
  if (!COMPROMISSO_TIPOS.includes(String(tipo))) return res.status(400).json({ error: "tipo inválido (use: callback, followup ou retomada)" });
  if (!data_hora || !String(data_hora).trim()) return res.status(400).json({ error: "data_hora é obrigatória" });
  try {
    const lead = await getRow("SELECT id, name FROM leads WHERE id = ?", [req.params.id]);
    if (!lead) return res.status(404).json({ error: "Lead não encontrado" });
    const cid = 'cp_' + Math.random().toString(36).substr(2, 9);
    await runQuery(
      "INSERT INTO compromissos (id, lead_id, tipo, data_hora, responsavel, cumprido, cumprido_em, notas, criado_em, criado_por) VALUES (?, ?, ?, ?, ?, 0, NULL, ?, ?, ?)",
      [cid, req.params.id, String(tipo), String(data_hora).trim(), String(responsavel || (req.user && req.user.name) || ''), String(notas || ''), new Date().toISOString(), (req.user && req.user.name) || '']
    );
    res.json(await getRow("SELECT * FROM compromissos WHERE id = ?", [cid]));
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// Atualiza um compromisso (marcar cumprido / reagendar / trocar responsável / notas).
app.patch('/api/compromissos/:id', authenticateToken, async (req, res) => {
  const { cumprido, data_hora, responsavel, notas } = req.body || {};
  try {
    const row = await getRow("SELECT * FROM compromissos WHERE id = ?", [req.params.id]);
    if (!row) return res.status(404).json({ error: "Compromisso não encontrado" });
    const upd = [], par = [];
    if (cumprido !== undefined) {
      const c = (cumprido === 1 || cumprido === '1' || cumprido === true) ? 1 : 0;
      upd.push("cumprido = ?"); par.push(c);
      upd.push("cumprido_em = ?"); par.push(c ? new Date().toISOString() : null);
    }
    if (data_hora !== undefined) { upd.push("data_hora = ?"); par.push(String(data_hora || '').trim()); }
    if (responsavel !== undefined) { upd.push("responsavel = ?"); par.push(String(responsavel || '')); }
    if (notas !== undefined) { upd.push("notas = ?"); par.push(String(notas || '')); }
    if (upd.length) { par.push(req.params.id); await runQuery("UPDATE compromissos SET " + upd.join(", ") + " WHERE id = ?", par); }
    res.json(await getRow("SELECT * FROM compromissos WHERE id = ?", [req.params.id]));
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// Exclui um compromisso (Administrador ou quem criou).
app.delete('/api/compromissos/:id', authenticateToken, async (req, res) => {
  try {
    const row = await getRow("SELECT * FROM compromissos WHERE id = ?", [req.params.id]);
    if (!row) return res.status(404).json({ error: "Compromisso não encontrado" });
    const isAdmin = req.user && req.user.role === 'Administrador';
    if (!isAdmin && row.criado_por !== ((req.user && req.user.name) || '')) {
      return res.status(403).json({ error: "Sem permissão para excluir este compromisso" });
    }
    await runQuery("DELETE FROM compromissos WHERE id = ?", [req.params.id]);
    res.json({ success: true, id: req.params.id });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// Agenda do dia: compromissos do dia (default hoje, fuso SP), pendentes primeiro, com dados do lead.
app.get('/api/compromissos', authenticateToken, async (req, res) => {
  try {
    const dia = String(req.query.dia || hojeSP()).slice(0, 10);
    const resp = String(req.query.responsavel || '').trim();
    let sql = "SELECT c.*, l.name AS lead_name, l.phone AS lead_phone, l.stage AS lead_stage FROM compromissos c LEFT JOIN leads l ON l.id = c.lead_id WHERE substr(c.data_hora, 1, 10) = ?";
    const par = [dia];
    if (resp) { sql += " AND c.responsavel = ?"; par.push(resp); }
    sql += " ORDER BY c.cumprido ASC, c.data_hora ASC";
    res.json(await allRows(sql, par));
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// execucao1 Sprint1 (2026-07-25): FILA DE TRABALHO priorizada por score × pendência.
// Ordena quem deve ser atendido AGORA (Adriana nota 9 sem boleto acima de lead novo nota 4),
// nunca por ordem de chegada. Só leitura — não altera nada.
app.get('/api/worklist', authenticateToken, async (req, res) => {
  try {
    const leads = await allRows("SELECT * FROM leads WHERE archived = 0 AND stage NOT IN ('convertida', 'declinado')");
    const pend = await allRows("SELECT * FROM compromissos WHERE cumprido = 0");
    const pendByLead = new Map();
    for (const c of pend) {
      if (!pendByLead.has(c.lead_id)) pendByLead.set(c.lead_id, []);
      pendByLead.get(c.lead_id).push(c);
    }
    const now = Date.now();
    const DUAS_HORAS = 2 * 3600 * 1000;
    const fmtHora = (iso) => { try { return String(iso).slice(11, 16) || String(iso).slice(0, 10); } catch (e) { return ''; } };
    const items = leads.map(l => {
      const motivo = [];
      let s = (Number(l.score) || 4) * 10;
      motivo.push('nota ' + (Number(l.score) || '4 (padrão)'));
      if (l.status_pagamento && l.status_pagamento !== 'lead') { s += 40; motivo.push('pagante (' + l.status_pagamento + ')'); }
      if (l.lastClientReply) { s += 30; motivo.push('cliente aguardando resposta'); }
      const cps = pendByLead.get(l.id) || [];
      let temVencido = false, temProximo = false, cpVencido = null, cpProximo = null;
      for (const c of cps) {
        const t = Date.parse(c.data_hora);
        if (isNaN(t)) continue;
        if (t < now && !temVencido) { temVencido = true; cpVencido = c; }
        else if (t >= now && t - now <= DUAS_HORAS && !temProximo) { temProximo = true; cpProximo = c; }
      }
      if (temVencido) { s += 25; motivo.push(cpVencido.tipo + ' vencido ' + fmtHora(cpVencido.data_hora)); }
      if (temProximo) { s += 15; motivo.push(cpProximo.tipo + ' às ' + fmtHora(cpProximo.data_hora)); }
      if (['urgente', 'followup'].includes(String(l.priority || ''))) { s += 10; motivo.push('prioridade ' + l.priority); }
      return {
        id: l.id, name: l.name, phone: l.phone, stage: l.stage,
        score: l.score, status_pagamento: l.status_pagamento || 'lead',
        responsible: l.responsible || '', compromissos_pendentes: cps.length,
        motivo, worklist_score: s
      };
    });
    items.sort((a, b) => b.worklist_score - a.worklist_score);
...
app.post('/api/leads', authenticateToken, async (req, res) => {
  const { name, phone, email, value, stage, source, company, priority, account, tags, destino_visto, tipo_visto, prazo_viagem, qtd_solicitantes, score, status_pagamento, bot_ativo } = req.body || {};
  ...
  try {
    ...
    // execucao1 Sprint1 (2026-07-25): campos opcionais de qualificação/score/pagamento no cadastro.
    {
      const _qUpd = [], _qPar = [];
      if (destino_visto !== undefined) { _qUpd.push("destino_visto = ?"); _qPar.push(String(destino_visto || '')); }
      if (tipo_visto !== undefined) { _qUpd.push("tipo_visto = ?"); _qPar.push(String(tipo_visto || '')); }
      if (prazo_viagem !== undefined) { _qUpd.push("prazo_viagem = ?"); _qPar.push(String(prazo_viagem || '')); }
      if (qtd_solicitantes !== undefined) { _qUpd.push("qtd_solicitantes = ?"); _qPar.push((qtd_solicitantes === null || qtd_solicitantes === '') ? null : (Number(qtd_solicitantes) || null)); }
      if (score !== undefined) { _qUpd.push("score = ?"); _qPar.push((score === null || score === '') ? null : Number(score)); }
      if (status_pagamento !== undefined) { _qUpd.push("status_pagamento = ?"); _qPar.push(String(status_pagamento)); }
      if (bot_ativo !== undefined) { _qUpd.push("bot_ativo = ?"); _qPar.push((bot_ativo === 0 || bot_ativo === '0' || bot_ativo === false) ? 0 : 1); }
      if (_qUpd.length) { _qPar.push(id); await runQuery("UPDATE leads SET " + _qUpd.join(", ") + " WHERE id = ?", _qPar); }
    }
...
async function reconcileReplyDots() {
  ...
  try {
    ...
    for (const l of leads) {
      ...
      // [BOLINHA-QUALQUER-RESPOSTA] (pedido do Henry, 2026-08-10 — substitui a regra de 08/08)
      // A bolinha marca CLIENTE SEM RETORNO. Qualquer resposta nossa apaga: humano OU IA. A regra
      // anterior mandava so a resposta humana apagar, e o efeito colateral apareceu em "Novos
      // Leads", onde quem atende e a IA: a coluna inteira ficava vermelha com as conversas
      // fluindo normalmente.
      // UNICA excecao, de proposito: a auto-resposta de fora do horario ("estamos fechados,
      // retornaremos"). Ela avisa, nao atende — se apagasse a bolinha, a demanda da madrugada
      // sumiria do radar antes de alguem ler.
      // ATENCAO: isto e a BOLINHA. A METRICA de tempo medio de atendimento
      // (GET /api/dashboard/response-time) segue contando SO resposta humana, como o Henry pediu —
      // sao duas regras diferentes de proposito, nao as unifique.
      const ndTs = Number(l.not_demand_ts) || 0;
      let _autoMsgTxt = null;
      try { const _b = await carregaBH(); if (_b && _b.message) _autoMsgTxt = String(_b.message).trim(); } catch (e) {}
      const _ultCliente = await getRow("SELECT MAX(timestamp) AS ts FROM messages WHERE conversationId = ? AND `from` = 'them'", [convoId]);
      const _ultResposta = _autoMsgTxt
        ? await getRow("SELECT MAX(timestamp) AS ts FROM messages WHERE conversationId = ? AND `from` = 'me' AND TRIM(COALESCE(text,'')) <> ?", [convoId, _autoMsgTxt])
        : await getRow("SELECT MAX(timestamp) AS ts FROM messages WHERE conversationId = ? AND `from` = 'me'", [convoId]);
      const tsCliente = Number(_ultCliente && _ultCliente.ts) || 0;
      const tsHumano = Number(_ultResposta && _ultResposta.ts) || 0;
...
// v589 [GERAR-AGORA] — o botao "Gerar" do chip do relatorio.
//
// Antes so existia a geracao das 8h e um disparo manual que rodava os DOIS ambientes,
// sem dizer quando terminava. Agora o pedido leva o ambiente escolhido no proprio chip
// (Pre, Pos ou Ambos) e a tela consulta /status para saber a hora de reler o relatorio.
let _relGerando = { rodando: false, env: '', iniciadoEm: 0, terminadoEm: 0, erro: '' };

app.post('/api/reports/daily/run', authenticateToken, async (req, res) => {
  if (req.user && req.user.role === 'Vendedor') return res.status(403).json({ detail: 'Sem permissão' });
  if (_relGerando.rodando) return res.status(409).json({ error: 'Já existe um relatório sendo gerado.', ..._relGerando });
  // 'pre', 'pos' ou vazio (= os dois). Qualquer outra coisa vira "os dois".
  const env = (req.body && ['pre', 'pos'].indexOf(String(req.body.env || '')) !== -1) ? String(req.body.env) : '';
  _relGerando = { rodando: true, env, iniciadoEm: Date.now(), terminadoEm: 0, erro: '' };
  setImmediate(async () => {
    try {
      const dia = dailyReports.hojeSPiso();
      if (env === 'pre' || env === 'pos') await dailyReports.runOne(env, dia);
      else await dailyReports.runDailyReports(dia);
      _relGerando.terminadoEm = Date.now();
      console.log('[daily-report] geração manual concluída (' + (env || 'pré e pós') + ').');
    } catch (e) {
      _relGerando.erro = String(e && e.message || e);
      console.error('[daily-report] run manual:', _relGerando.erro);
    } finally { _relGerando.rodando = false; }
  });
  res.status(202).json({ queued: true, ..._relGerando });
});

app.get('/api/reports/daily/run/status', authenticateToken, async (req, res) => {
  res.json(_relGerando);
});
// execucao2 (2026-07-25): prazo de análise do relatório (horas) — app_settings, default 48.
async function dailyPrazoHoras() {
  try {
    const row = await getRow("SELECT value FROM app_settings WHERE key = 'daily_report_prazo_horas'");
    const n = parseInt(row && row.value, 10);
    return (Number.isFinite(n) && n >= 1 && n <= 720) ? n : 48;
  } catch (e) { return 48; }
}
// Salvar o prazo — mesma regra do /run (só quem não é Vendedor).
app.post('/api/reports/daily/prazo', authenticateToken, async (req, res) => {
  if (req.user && req.user.role === 'Vendedor') return res.status(403).json({ detail: 'Sem permissão' });
  const n = parseInt(req.body && req.body.horas, 10);
  if (!Number.isFinite(n) || n < 1 || n > 720) return res.status(400).json({ error: 'Prazo inválido — use um número de horas entre 1 e 720.' });
  try {
    await runQuery("INSERT INTO app_settings (key, value) VALUES ('daily_report_prazo_horas', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value", [String(n)]);
    res.json({ ok: true, horas: n });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
// Marcar como ANALISADO (quando o usuário abre o relatório completo) — qualquer usuário logado.
app.post('/api/reports/daily/:id/analyzed', authenticateToken, async (req, res) => {
  try {
    await runQuery("UPDATE ai_reports SET analyzed_at = ? WHERE id = ? AND type IN ('daily-pre','daily-pos') AND analyzed_at IS NULL", [new Date().toISOString(), req.params.id]);
    res.json({ ok: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
// Histórico (sem o corpo, para a lista ficar leve). env=pre|pos; default = ambiente do usuário.
app.get('/api/reports/daily', authenticateToken, async (req, res) => {
  try {
    const env = ['pre', 'pos'].includes(String(req.query.env)) ? String(req.query.env) : ((await userIsPos(req)) ? 'pos' : 'pre');
    res.json(await dailyReports.listDaily(env, Number(req.query.limit) || 30));
  } catch (e) { res.status(500).json({ error: e.message }); }
});
// Mais recente CONCLUÍDO do ambiente do usuário (card do topo do Dashboard).
app.get('/api/reports/daily/latest', authenticateToken, async (req, res) => {
  try {
    const qEnv = ['pre', 'pos'].includes(String(req.query.env)) ? String(req.query.env) : null;
    const prazo = await dailyPrazoHoras();
    // execucao4 (pedido do Henry): usuário NÃO-Vendedor (admin) sem ?env explícito recebe o
    // relatório COMBINADO — seção Pré-venda + seção Pós-venda num único documento (type
    // 'daily-combo', ids com os 2 relatórios p/ marcar analisado). Vendedor: só o próprio ambiente.
    if (!qEnv && !(req.user && req.user.role === 'Vendedor')) {
      const rPre = await dailyReports.latestDaily('pre');
      const rPos = await dailyReports.latestDaily('pos');
      if (!rPre && !rPos) return res.status(404).json({ error: 'Nenhum relatório diário gerado ainda.' });
      if (rPre && rPos) {
        const base = (Date.parse(rPos.created_at || 0) >= Date.parse(rPre.created_at || 0)) ? rPos : rPre;
        return res.json(Object.assign({}, base, {
          combo: 1, ids: [rPre.id, rPos.id], type: 'daily-combo',
          result: String(rPre.result || '') + '\n\n---\n\n' + String(rPos.result || ''),
          analyzed_at: (rPre.analyzed_at && rPos.analyzed_at) ? (rPre.analyzed_at > rPos.analyzed_at ? rPre.analyzed_at : rPos.analyzed_at) : null,
          prazo_horas: prazo
        }));
      }
      const only = rPre || rPos;
      return res.json(Object.assign({}, only, { combo: 1, ids: [only.id], prazo_horas: prazo }));
    }
    const env = qEnv || ((await userIsPos(req)) ? 'pos' : 'pre');
    const r = await dailyReports.latestDaily(env);
    if (!r) return res.status(404).json({ error: 'Nenhum relatório diário gerado ainda.' });
    res.json(Object.assign({}, r, { prazo_horas: prazo }));
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.get('/api/reports/daily/:id', authenticateToken, async (req, res) => {
  try {
    const r = await dailyReports.getDaily(req.params.id);
    if (!r) return res.status(404).json({ error: 'Relatório não encontrado.' });
    res.json(Object.assign({}, r, { prazo_horas: await dailyPrazoHoras() }));
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Scheduler: a cada 60s confere a hora em America/Sao_Paulo; às 8h gera os relatórios do dia
// (1x por dia — trava por data em app_settings['daily_report_last_run'], sobrevive a restart).
setInterval(async () => {
  try {
    const hora = new Intl.DateTimeFormat('en-US', { timeZone: 'America/Sao_Paulo', hour: '2-digit', hourCycle: 'h23' }).format(new Date());
    if (parseInt(hora, 10) !== 8) return;
    const hoje = dailyReports.hojeSPiso();
    const row = await getRow("SELECT value FROM app_settings WHERE key = 'daily_report_last_run'");
    if (row && row.value === hoje) return;
    await runQuery("INSERT INTO app_settings (key, value) VALUES ('daily_report_last_run', ?) ON CONFLICT(key) DO UPDATE SET value = excluded.value", [hoje]);
    console.log('[daily-report] 8h em São Paulo — gerando relatórios do dia', hoje);
    dailyReports.runDailyReports(hoje).catch((e) => console.error('[daily-report] scheduler:', e && e.message));
  } catch (e) { console.error('[daily-report] scheduler tick:', e && e.message); }
}, 60 * 1000);
...
app.get('/api/dashboard/response-time', authenticateToken, async (req, res) => {
  try {
    // [RT-COMERCIAL] escopo=hoje: janela do inicio do expediente de hoje ate agora — e a janela
    // do indicador do cabecalho, que perdeu o combo de periodo. 'minutes' continua valendo para
    // quem chamar a rota do jeito antigo.
    const _bh = await carregaBH();
    const now = Date.now();
    const _escopoHoje = String(req.query.escopo || '') === 'hoje';
    const minutes = Math.max(1, Math.min(60 * 24 * 60, parseInt(req.query.minutes, 10) || 60)); // 1 min .. 60 dias
    const since = _escopoHoje ? inicioExpedienteHoje(now, _bh) : (now - minutes * 60 * 1000);
    const BUFFER_MS = 6 * 3600 * 1000; // contexto p/ detectar o início de demandas próximas de "since"

    // [RT-COMERCIAL] Recorte = subcoluna 1 do Tratamento inicial ("Lead respondendo"): o card so
    // entra nela quando ha mensagem do cliente esperando resposta HUMANA. Fora: os da coluna-ponte
    // (bridge=1, que pertencem ao outro ambiente) e os em on-hold (atendimento suspenso de proposito
    // — cobrar tempo de resposta deles distorceria a media).
    const leads = await allRows(
      "SELECT phone, whatsapp_jid FROM leads WHERE archived = 0 AND stage = 'tratamento' " +
      "AND COALESCE(bridge,0) <> 1 AND COALESCE(priority,'') <> 'onhold'");
    ...
    for (const m of msgs) {
      ...
      if (m.from === 'them') {
        ...
        if (isHumanReply(m) && demandTs !== null) {
          // [RT-COMERCIAL] mede so o tempo DENTRO do expediente (msUteis), nao o relogio de parede.
          if (demandTs >= since && ts >= demandTs) { totalMs += msUteis(demandTs, ts, _bh); count++; }
    ...
    res.json({ avgMs: count ? Math.round(totalMs / count) : null, count, pending, minutes,
      escopo: _escopoHoje ? 'hoje' : 'minutes', comercial: 1, since });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// [RT-GRAFICO] Evolucao do tempo medio de atendimento POR DIA, no periodo do dashboard.
// Mesmo recorte e mesma medicao do indicador do cabecalho: so a fila "Lead respondendo",
// so horario comercial, sem contar IA nem auto-resposta. O dia de um atendimento e o dia
// da mensagem do CLIENTE (e quando o relogio comecou a correr).
app.get('/api/dashboard/response-time-daily', authenticateToken, async (req, res) => {
  try {
    const range = daysRangeSP(req.query.from, req.query.to, 15);
    const sinceMs = Date.parse(range[0].iso + 'T00:00:00Z') + 3 * 3600 * 1000;
    const untilMs = Date.parse(range[range.length - 1].iso + 'T00:00:00Z') + 3 * 3600 * 1000 + 24 * 3600 * 1000;
    const BUFFER_MS = 6 * 3600 * 1000;
    const _bh = await carregaBH();
    const keyBr = (ms) => new Date(ms - 3 * 3600 * 1000).toISOString().slice(0, 10);
    const vazio = () => range.map(d => { const p = d.iso.split('-'); return { date: d.iso, label: p[2] + '/' + p[1], avgMs: null, count: 0 }; });

    const leads = await allRows(
      "SELECT phone, whatsapp_jid FROM leads WHERE archived = 0 AND stage = 'tratamento' " +
      "AND COALESCE(bridge,0) <> 1 AND COALESCE(priority,'') <> 'onhold'");
    if (!leads.length) return res.json({ days: vazio(), avgMs: null, count: 0 });
    ...
    if (!convIds.length) return res.json({ days: vazio(), avgMs: null, count: 0 });

    let autoMsg = null;
    try { if (_bh && _bh.message) autoMsg = String(_bh.message).trim(); } catch (e) {}
    const isHumanReply = (m) => m.from === 'me' && Number(m.ai) === 0 && !(autoMsg && String(m.text || '').trim() === autoMsg);

    const ph = convIds.map(() => '?').join(',');
    const msgs = await allRows(
      "SELECT conversationId, `from`, COALESCE(ai,0) AS ai, text, timestamp FROM messages WHERE conversationId IN (" + ph + ") AND timestamp >= ? AND timestamp < ? ORDER BY conversationId, timestamp ASC",
      [...convIds, sinceMs - BUFFER_MS, untilMs + BUFFER_MS]
    );

    const acc = {}; // dia -> { soma, n }
    let curConv = null, demandTs = null;
    for (const m of msgs) {
      if (m.conversationId !== curConv) { curConv = m.conversationId; demandTs = null; }
      const ts = m.timestamp || 0;
      if (m.from === 'them') { demandTs = ts; }
      else if (m.from === 'me' && isHumanReply(m) && demandTs !== null) {
        if (ts >= demandTs && demandTs >= sinceMs && demandTs < untilMs) {
          const k = keyBr(demandTs);
          const a = acc[k] || (acc[k] = { soma: 0, n: 0 });
          a.soma += msUteis(demandTs, ts, _bh); a.n++;
        }
        demandTs = null;
      }
    }
    ...
    const days = range.map(d => {
      const p = d.iso.split('-');
      const a = acc[d.iso];
      return { date: d.iso, label: p[2] + '/' + p[1], avgMs: (a && a.n) ? Math.round(a.soma / a.n) : null, count: (a && a.n) || 0 };
    });
    const somaGeral = Object.values(acc).reduce((s, a) => s + a.soma, 0);
    const nGeral = Object.values(acc).reduce((s, a) => s + a.n, 0);
    res.json({ days, avgMs: nGeral ? Math.round(somaGeral / nGeral) : null, count: nGeral });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
const WELCOME_TEXT = 'Olá! Aqui é da Vale Visto — recebemos seu contato e queremos analisar o seu caso. Pode nos contar sua situação?';
  ...
  try {
    ...
    for (const l of eligible) {
      ...
      try {
        ...
        // [CANAL-CERTO v611] Quem chega pelo Instagram e atendido PELO INSTAGRAM.
        // Medido em 14/08/2026: 43 conversas ig:, 71 mensagens enviadas por linha de
        // WhatsApp desde 04/07 e ZERO entregues. O lead do Instagram nao tem telefone
        // (25 leads, nenhum com numero) e a Meta nao fornece esse dado por API.
        if (String(jidLead).indexOf('ig:') === 0 || String((convo && convo.whatsapp_jid) || '').indexOf('ig:') === 0) {
          console.log('[canal-certo] "' + (l.name || l.id) + '": lead do Instagram — nao entra na fila de WhatsApp.');
          continue;
        }
        if (!convo) {
          const convoId = 'c_' + Math.random().toString(36).substr(2, 9);
...
// BOAS-VINDAS AUTOMÁTICAS (Henry, 2026-08-12 — "iniciar a conversa quando o cliente NÃO manda mensagem,
// SEMPRE"): mesma lógica da rota manual acima, mas disparada SOZINHA (no boot + a cada 10 min). Continua
// respeitando as travas de segurança do WhatsApp (horário comercial, linha conectada, antiban:
// pacing/caps/warm-up) — "sempre" = todo lead elegível recebe a 1ª mensagem, dentro do que é seguro para
// não queimar o número. Idempotente (só pega lead em 'novo', sem linha, sem conversa) e com teto por rodada.
let _welcomeAutoRunning = false;
async function welcomeAutoSweep() {
  if (_welcomeAutoRunning) return;
  _welcomeAutoRunning = true;
  try {
    if (!_inBusinessHours()) return;                       // Seg–Sex 9–18h, Sáb 9–13h
    const accountId = await resolveWelcomeAccount();
    if (!accountId) { console.log('[welcome auto] linha padrão (12) 98248-3094 não está entre as contas — nada enviado.'); return; }
    const isOpen = !!(sessions[accountId] && sessions[accountId].ws && sessions[accountId].ws.isOpen);
    if (!isOpen) { console.log('[welcome auto] linha (12) 98248-3094 desconectada — aguardando religar.'); return; }
    const eligible = await _eligibleWelcomeLeads();
    if (!eligible.length) return;
    const cap = 25;                                        // teto por rodada (a cada 10 min)
    const num = '+' + WELCOME_LINE_DIGITS;
    let sent = 0;
    for (const l of eligible) {
      if (sent >= cap) break;
      if (!antiban.isBusinessHours()) break;
      const _g = await antiban.canSendProactive(accountId);
      if (!_g.ok) { console.log('[welcome auto] número protegido: ' + _g.reason + ' — pausado nesta rodada.'); break; }
      try {
        const digits = _digits(l.phone); const tail = digits.slice(-8); const jidLead = l.whatsapp_jid || '';
        let convo = null;
        if (jidLead) convo = await getRow("SELECT * FROM conversations WHERE whatsapp_jid = ? LIMIT 1", [jidLead]);
        ...
        logLeadHistory({ leadId: l.id, phone: l.phone, name: l.name, type: 'nota', detail: 'Boas-vindas automáticas enviadas pela linha (12) 98248-3094.' });
        sent++;
      } catch (e) { console.error('[welcome auto]', (l.name || l.id) + ':', e && e.message); }
    }
    if (sent) console.log(`[welcome auto] ${sent} lead(s) sem conversa receberam a 1ª mensagem automaticamente.`);
  } catch (e) { console.error('[welcome auto sweep]', e && e.message); }
  finally { _welcomeAutoRunning = false; }
}
...
async function aiFollowUpSweep() {
  ...
  try {
    ...
    // =======================================================================
    // v588 [FILA-JUSTA] — quem espera ha mais tempo vai primeiro.
    //
    // Antes a consulta nao tinha ORDER BY: o banco devolvia os leads na ordem em
    // que foram GRAVADOS, entao quem entrou primeiro no CRM saia primeiro. Um card
    // de 07/08 tinha 197 leads na frente e so seria alcancado seis dias depois.
    // Agora a fila e ordenada pela ultima mensagem da conversa: o mais esquecido
    // vai na frente, e quem acabou de receber follow-up cai para o fim sozinho.
    //
    // De quebra, a ultima mensagem de cada conversa passa a ser lida de UMA vez
    // (uma agregacao) em vez de uma consulta por lead dentro do laco.
    // =======================================================================
    const ultimaPorConversa = new Map();
    try {
      const _u = await allRows(
        "SELECT m.conversationId AS cid, m.`from` AS de, m.timestamp AS ts FROM messages m " +
        "JOIN (SELECT conversationId, MAX(timestamp) AS mx FROM messages GROUP BY conversationId) x " +
        "  ON x.conversationId = m.conversationId AND x.mx = m.timestamp"
      );
      for (const r of (_u || [])) if (!ultimaPorConversa.has(r.cid)) ultimaPorConversa.set(r.cid, r);
    } catch (e) { console.error('[IA follow-up] mapa da ultima mensagem:', e && e.message); }

    // Monta a lista de quem esta REALMENTE pronto, com a espera ja calculada.
    const prontos = [];
    for (const l of leads) {
      // O gate de coluna so vale para o Tratamento; lead em 'novo' entra sempre.
      if (l.stage !== 'novo') {
      ...
      if (_posLines.has(conv.account)) continue;              // pos-venda (2030): sem follow-up da IA
      const last = ultimaPorConversa.get(conv.id);
      if (!last || last.de !== 'me') continue;                // so quando NOS falamos por ultimo
      if (Date.now() - (last.ts || 0) < horasMs) continue;    // respeita o intervalo minimo
      prontos.push({ l, conv, esperaDesde: Number(last.ts) || 0 });
    }
    prontos.sort((a, b) => a.esperaDesde - b.esperaDesde);    // mais esquecido primeiro

    // -----------------------------------------------------------------------
    // RITMO: espalhado pelo dia, nao em rajada.
    // A cota da rodada e o que falta dividido pelas rodadas que ainda cabem ate o
    // fim da janela. Com a fila grande, sai mais por rodada; com a fila curta, sai
    // pouco e devagar. O teto existe para nunca virar disparo em massa.
    // -----------------------------------------------------------------------
    const TETO_RODADA = 30;
    let porRodada = TETO_RODADA;
    try {
      const _ag = new Date();
      const minutosAteOFim = Math.max(30, (17 - _ag.getHours()) * 60 - _ag.getMinutes());
      const rodadasQueFaltam = Math.max(1, Math.floor(minutosAteOFim / 30));
      porRodada = Math.max(1, Math.min(TETO_RODADA, Math.ceil(prontos.length / rodadasQueFaltam)));
    } catch (e) {}

    // Alterna entre as linhas: sem isto, uma linha com 229 leads na fila consumia a
    // rodada inteira e as outras quatro ficavam paradas. Com o rodizio, todas andam.
    const porLinha = new Map();
    for (const p of prontos) {
      if (!porLinha.has(p.conv.account)) porLinha.set(p.conv.account, []);
      porLinha.get(p.conv.account).push(p);
    }
    const fila = [];
    { const baldes = [...porLinha.values()];
      for (let i = 0; fila.length < prontos.length; i++) {
        let mexeu = false;
        for (const b of baldes) if (b[i]) { fila.push(b[i]); mexeu = true; }
        if (!mexeu) break;
      } }

    const linhasNoTeto = new Set();   // linha que bateu o cap: sai da rodada, sem derrubar as outras
    let processed = 0;
    console.log(`[IA follow-up] fila: ${prontos.length} esperando; cota desta rodada: ${porRodada}.`);
    for (const _item of fila) {
      const l = _item.l, conv = _item.conv;
      if (processed >= porRodada) { console.log(`[IA follow-up] cota da rodada (${porRodada}) cumprida; o restante segue na proxima.`); break; }
      if (linhasNoTeto.has(conv.account)) continue;
      try {
        // v588: coluna, conversa, ultima mensagem e intervalo ja foram conferidos ao
        // montar a fila la em cima — aqui so sobra o que precisa ser relido na hora do envio.
        ...
        // WhatsApp desde 04/07 e ZERO entregues. O lead do Instagram nao tem telefone
        // (25 leads, nenhum com numero) e a Meta nao fornece esse dado por API.
        if (String((conv && conv.whatsapp_jid) || '').indexOf('ig:') === 0) {
          console.log('[canal-certo] "' + l.name + '": lead do Instagram — follow-up segue pelo Direct.');
          continue;
        }
        ...
        // v588: cap de UMA linha tira so aquela linha da rodada. Antes era um break e
        // as outras quatro linhas paravam junto, mesmo com folga de sobra.
        if (!_gate.ok) { console.log(`[IA follow-up] linha ${conv.account}: ${_gate.reason} — segue com as outras linhas.`); linhasNoTeto.add(conv.account); continue; }
        ...
        processed++;
        const _diasParado = Math.round((Date.now() - _item.esperaDesde) / 86400000);
        console.log(`[IA follow-up] "${l.name}": tentativa ${tentativa} enviada (${processed}/${porRodada}) — parado ha ${_diasParado} dia(s).`);
  ...
  try {
    ...
    for (const l of leads) {
      try {
        ...
        // [R12-ABRE-SEM-CONVERSA] Lead que chegou por formulario, e-mail ou anuncio nunca teve
        // conversa de WhatsApp — e antes a rotina simplesmente pulava, entao ele ficava parado em
        // "Novos Leads" para sempre, sem receber uma linha sequer. Agora a conversa e CRIADA aqui e
        // a IA faz o primeiro contato. Sem telefone utilizavel ou sem linha pre conectada, ai sim
        // nao ha o que fazer: pula.
        let _criarConv = false;
        if (!conv || !conv.account) {
          const _dig = norm(l.phone);
          if (_dig.length < 8) continue;
          const _isOpen = (a) => sessions[a] && sessions[a].ws && sessions[a].ws.isOpen;
          let _acc = (l.account && _isOpen(l.account) && !_pos.has(l.account)) ? l.account : '';
          if (!_acc) _acc = Object.keys(sessions || {}).find(a => _isOpen(a) && !_pos.has(a)) || '';
          if (!_acc) continue;
          // A conversa so e criada no INSTANTE do envio (mais abaixo). Criar aqui encheria a guia
          // WhatsApp de conversas vazias toda vez que a rodada caisse fora do horario comercial.
          conv = { id: null, account: _acc, phone: String(l.phone || ''), whatsapp_jid: l.whatsapp_jid || null };
          _criarConv = true;
        }
        if (_pos.has(conv.account)) continue;
        const last = conv.id ? await getRow("SELECT `from`, timestamp FROM messages WHERE conversationId = ? ORDER BY timestamp DESC LIMIT 1", [conv.id]) : null;
        // [R12-ABRE] Lead que NUNCA escreveu (conversa sem nenhuma mensagem) tambem entra na
        // cadencia: a IA abre a conversa (decisao do Henry, 2026-08-10). Antes a rotina exigia uma
        // mensagem NOSSA para contar o tempo, entao quem chegou pelo anuncio e nunca falou ficava
        // parado em "Novos Leads" para sempre, sem receber nada.
        const _semNenhuma = !last;
        if (!_semNenhuma && last.from !== 'me') continue; // cliente falou por ultimo: nao e caso de nudge
        // Sem mensagem nenhuma: o relogio corre da CRIACAO do lead, nao da ultima mensagem.
        const _refTs = _semNenhuma ? Date.parse(l.createdAt || '') || Date.now() : (Number(last.timestamp) || 0);
        const elapsed = Date.now() - _refTs;
        ...
        if ((Number(l.novo_nudge_count) || 0) >= NOVO_NUDGE_MAX) {
          if (elapsed >= MOVE_MS) {
            // [R11-SILENCIO] prioridade_fila = 4 = "veio de Novos Leads e NUNCA respondeu" — o
            // frontend coloca esse card na subcoluna "Lead sem responder" (decisao do Henry,
            // 2026-08-10). O valor 3, usado antes, continua caindo em "Lead respondendo": assim os
            // cards antigos ficam onde estao e so os novos seguem a regra nova.
            await runQuery("UPDATE leads SET stage = 'tratamento', prioridade_fila = 4, coluna_origem = 'novo' WHERE id = ? AND stage = 'novo'", [l.id]);
            const note = '📥 [Automático] Movido para "Lead sem responder": foram feitas ' + NOVO_NUDGE_MAX + ' tentativas de contato em "Novos Leads" e o cliente ficou mais 24h sem responder.';
            await runQuery("UPDATE leads SET comments = TRIM(COALESCE(comments,'') || char(10) || ?) WHERE id = ?", [note, l.id]);
            try { await logLeadHistory({ leadId: l.id, phone: l.phone, name: l.name, type: 'movimentacao', detail: 'Movido para "Lead sem responder" — ' + NOVO_NUDGE_MAX + ' tentativas em "Novos Leads" sem resposta', meta: { to: 'tratamento', subcoluna: 'sem_responder', r2b: 1, r11: 1 } }); } catch (e) {}
            try { const full = await getRow("SELECT * FROM leads WHERE id = ?", [l.id]); if (full) sendWebhook('lead.stage_changed', { ...full, tags: full.tags ? JSON.parse(full.tags) : [] }); } catch (e) {}
        ...
        // [R12-ABRE] Primeira fala em conversa vazia: apresentacao, nao cobranca. Os textos de
        // nudge pressupoem uma mensagem anterior ("confirmar se recebeu a mensagem acima") e ficariam
        // sem sentido para quem nunca recebeu nada.
        const texto = (_semNenhuma && count === 0)
          ? ('Olá' + (nome ? ' ' + nome : '') + '! Aqui é da Vale Visto. Recebemos o seu contato e queremos te ajudar com o seu processo de visto. Pode me contar qual país você pretende visitar?')
          : _textos[Math.min(count, _textos.length - 1)];
        ...
        // [R12-ABRE-SEM-CONVERSA] agora sim: cria a conversa, no instante do envio
        if (_criarConv && !conv.id) {
          const _cid = 'c_' + Math.random().toString(36).substr(2, 9);
          const _nm = l.name || norm(l.phone);
          const _hora = new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit', timeZone: 'America/Sao_Paulo' });
          await runQuery(
            "INSERT INTO conversations (id, account, name, phone, avatar, lastTime, unread, online, whatsapp_jid, archived) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            [_cid, conv.account, _nm, String(l.phone || ''), String(_nm).slice(0, 2).toUpperCase(), _hora, 0, 0, l.whatsapp_jid || null, 0]
          );
          conv.id = _cid;
          convs.push({ id: _cid, account: conv.account, phone: String(l.phone || ''), whatsapp_jid: l.whatsapp_jid || null });
          try { bustConversationsCache(); } catch (e) {}
          try {
            const _accRow = await getRow("SELECT number FROM whatsapp_accounts WHERE id = ?", [conv.account]);
            if (_accRow && _accRow.number) await runQuery("UPDATE leads SET account = ?, recv_number = ? WHERE id = ?", [conv.account, _accRow.number, l.id]);
          } catch (e) {}
          console.log('[cadência novo] "' + l.name + '": conversa criada na linha ' + conv.account + ' — primeiro contato da IA.');
        }
        // [CANAL-CERTO v611] Quem chega pelo Instagram e atendido PELO INSTAGRAM.
        // Medido em 14/08/2026: 43 conversas ig:, 71 mensagens enviadas por linha de
        // WhatsApp desde 04/07 e ZERO entregues. O lead do Instagram nao tem telefone
        // (25 leads, nenhum com numero) e a Meta nao fornece esse dado por API.
        if (String((conv && conv.whatsapp_jid) || '').indexOf('ig:') === 0) {
          console.log('[canal-certo] "' + l.name + '": lead do Instagram — cadencia nao usa WhatsApp.');
          continue;
        }
...
// ===== [API-V1] API DE SAIDA — dados do CRM para agencias e BI ==============================
// Por que uma API nova em vez de abrir o export que ja existia: o GET /integrations/leads devolve
// "SELECT * FROM leads" — a base inteira, com nome, telefone, e-mail e comentarios internos, sem
// paginacao, autenticado pela MESMA chave que a agencia usa para ENVIAR leads. Quem envia baixa
// tudo, e revogar uma coisa revoga a outra.
// Aqui: chave por parceiro, escopos, sem dado pessoal legivel, paginacao por cursor e registro de
// cada acesso. O contato sai em HASH SHA-256 (formato que Meta e Google pedem para conversao
// offline e publico personalizado) — serve para a agencia casar o lead sem nunca ler o telefone.
const crypto_v1 = require('crypto');
const V1_MAX_DIAS = 92;
const V1_LIMITE_PADRAO = 100, V1_LIMITE_TETO = 500;

function v1Hash(txt) {
  const t = String(txt || '').trim().toLowerCase();
  if (!t) return null;
  return crypto_v1.createHash('sha256').update(t).digest('hex');
}
function v1HashTelefone(tel) {
  const d = String(tel || '').replace(/\D/g, '');
  if (d.length < 8) return null;
  // padrao E.164 sem o "+", como Meta e Google esperam
  return crypto_v1.createHash('sha256').update(d.length <= 11 ? ('55' + d) : d).digest('hex');
}
function v1Erro(res, status, codigo, mensagem, campo) {
  return res.status(status).json({ erro: { codigo, mensagem, campo: campo || undefined } });
}
// Limite de taxa em memoria: 60 chamadas por minuto por chave.
const _v1Rate = new Map();
function v1RateOk(keyId) {
  const agora = Date.now(), janela = 60000, teto = 60;
  const r = _v1Rate.get(keyId) || { ini: agora, n: 0 };
  if (agora - r.ini > janela) { r.ini = agora; r.n = 0; }
  r.n++; _v1Rate.set(keyId, r);
  return { ok: r.n <= teto, restante: Math.max(0, teto - r.n), reset: Math.ceil((r.ini + janela - agora) / 1000) };
}
// Autenticacao: Bearer <chave>. Confere o hash, o escopo e registra o uso.
function v1Auth(escopoExigido) {
  return async (req, res, next) => {
    try {
      const h = String(req.headers['authorization'] || '');
      const chave = h.startsWith('Bearer ') ? h.slice(7).trim() : (req.query.key || '');
      if (!chave) return v1Erro(res, 401, 'sem_chave', 'Envie a chave no cabeçalho Authorization: Bearer <chave>.');
      const hash = crypto_v1.createHash('sha256').update(String(chave)).digest('hex');
      const k = await getRow("SELECT * FROM api_partner_keys WHERE key_hash = ? AND ativa = 1", [hash]);
      if (!k) return v1Erro(res, 401, 'chave_invalida', 'Chave inválida ou revogada.');
      const escopos = String(k.escopos || '').split(',').map(x => x.trim()).filter(Boolean);
      if (escopoExigido && escopos.indexOf(escopoExigido) === -1) {
        return v1Erro(res, 403, 'sem_permissao', 'Esta chave não tem permissão para "' + escopoExigido + '".');
      }
      const rt = v1RateOk(k.id);
      res.setHeader('X-RateLimit-Restante', String(rt.restante));
      if (!rt.ok) { res.setHeader('Retry-After', String(rt.reset)); return v1Erro(res, 429, 'limite_excedido', 'Limite de 60 requisições por minuto. Tente em ' + rt.reset + 's.'); }
      req.v1 = { key: k, escopos, pii: escopos.indexOf('pii:ler') !== -1 };
      try { await runQuery("UPDATE api_partner_keys SET ultimo_uso_em = ? WHERE id = ?", [new Date().toISOString(), k.id]); } catch (e) {}
      next();
    } catch (e) { return v1Erro(res, 500, 'erro_interno', e && e.message); }
  };
}
async function v1Log(req, registros, status) {
  try {
    await runQuery("INSERT INTO api_access_log (ts, key_id, parceiro, rota, parametros, registros, status, ip) VALUES (?,?,?,?,?,?,?,?)",
      [new Date().toISOString(), (req.v1 && req.v1.key.id) || null, (req.v1 && req.v1.key.nome) || null,
       req.path, JSON.stringify(req.query || {}).slice(0, 500), registros, status,
       String(req.headers['x-forwarded-for'] || req.ip || '').slice(0, 60)]);
  } catch (e) {}
}
// Periodo obrigatorio, no maximo 92 dias — evita o "puxa tudo" a cada consulta.
function v1Periodo(req) {
  const ok = (x) => /^\d{4}-\d{2}-\d{2}$/.test(String(x || ''));
  const desde = String(req.query.desde || ''), ate = String(req.query.ate || '');
  if (!ok(desde) || !ok(ate)) return { erro: ['periodo_obrigatorio', 'Informe "desde" e "ate" no formato AAAA-MM-DD.', 'desde'] };
  if (desde > ate) return { erro: ['periodo_invalido', 'A data inicial é maior que a final.', 'desde'] };
  const dias = (Date.parse(ate) - Date.parse(desde)) / 86400000;
  if (dias > V1_MAX_DIAS) return { erro: ['periodo_invalido', 'O período não pode passar de ' + V1_MAX_DIAS + ' dias.', 'ate'] };
  return { desde, ate };
}
function v1Limite(req) {
  const n = parseInt(req.query.limite, 10);
  return Math.max(1, Math.min(V1_LIMITE_TETO, isNaN(n) ? V1_LIMITE_PADRAO : n));
}
function v1Utm(l) {
  let t = {}; try { t = l.tracking ? JSON.parse(l.tracking) : {}; } catch (e) { t = {}; }
  const u = {};
  Object.keys(t || {}).forEach(k => { if (/^utm_/i.test(k)) u[k.toLowerCase().replace(/^utm_/, '')] = String(t[k]); });
  return { utm: u, gclid: t.gclid || null, fbclid: t.fbclid || null, landing_page: t.landing_page || null };
}
const V1_ETAPAS = { novo: 'novo', tratamento: 'em_atendimento', convertida: 'convertida', declinado: 'perdida', clientes_antigos: 'em_atendimento' };
function v1Lead(l, comPii) {
  const tk = v1Utm(l);
  let tags = []; try { tags = l.tags ? JSON.parse(l.tags) : []; } catch (e) {}
  const o = {
    id: l.id,
    criado_em: l.createdAt ? new Date(l.createdAt).toISOString() : null,
    origem: l.source || null,
    campanha: tk.utm.campaign || null,
    conjunto: tk.utm.content || tk.utm.term || null,
    utm: tk.utm,
    gclid: tk.gclid, fbclid: tk.fbclid,
    servico: Array.isArray(tags) && tags.length ? tags[0] : null,
    etapa: V1_ETAPAS[l.stage] || l.stage || null,
    convertido: l.stage === 'convertida',
    convertido_em: l.stage === 'convertida' ? (l.sale_date || (l.contracted_at ? String(l.contracted_at).slice(0, 10) : null)) : null,
    valor: l.stage === 'convertida' ? (Number(l.value) || 0) : null,
    motivo_perda: l.stage === 'declinado' ? (l.decline_reason || null) : null,
    // hashes: servem para casar publico no Meta/Google sem expor o contato
    email_sha256: v1Hash(l.email),
    telefone_sha256: v1HashTelefone(l.phone)
  };
  // So com o escopo pii:ler (desligado por padrao) o contato sai legivel.
  if (comPii) { o.nome = l.name || null; o.email = l.email || null; o.telefone = l.phone || null; }
  return o;
}
function v1Csv(linhas) {
...
function v1Responde(req, res, dados, extra) {
  const csv = /text\/csv/.test(String(req.headers.accept || '')) || String(req.query.formato || '') === 'csv';
  if (csv) {
    res.setHeader('Content-Type', 'text/csv; charset=utf-8');
    res.setHeader('Content-Disposition', 'attachment; filename="' + req.path.split('/').pop() + '.csv"');
    return res.send('﻿' + v1Csv(dados));
  }
  res.json(Object.assign({ dados }, extra || {}));
}

app.get('/api/v1/health', v1Auth(null), async (req, res) => {
  const ult = await getRow("SELECT MAX(createdAt) m FROM leads");
  res.json({ ok: true, versao: 'v1', parceiro: req.v1.key.nome, escopos: req.v1.escopos, ultimo_lead_em: (ult && ult.m) || null });
  v1Log(req, 1, 200);
});

app.get('/api/v1/leads', v1Auth('leads:ler'), async (req, res) => {
  const p = v1Periodo(req);
  if (p.erro) { v1Log(req, 0, 400); return v1Erro(res, 400, p.erro[0], p.erro[1], p.erro[2]); }
  const lim = v1Limite(req);
  const cond = ["archived = 0", "substr(createdAt,1,10) >= ?", "substr(createdAt,1,10) <= ?"];
  const par = [p.desde, p.ate];
  if (req.query.origem) { cond.push("LOWER(COALESCE(source,'')) = LOWER(?)"); par.push(String(req.query.origem)); }
  if (req.query.etapa) {
    const inv = Object.keys(V1_ETAPAS).filter(k => V1_ETAPAS[k] === String(req.query.etapa));
    if (inv.length) { cond.push("stage IN (" + inv.map(() => '?').join(',') + ")"); par.push(...inv); }
  }
  if (String(req.query.convertido) === 'true') cond.push("stage = 'convertida'");
  // Cursor: id do ultimo registro da pagina anterior (ordem estavel por createdAt+id).
  let cursor = null;
  try { if (req.query.cursor) cursor = JSON.parse(Buffer.from(String(req.query.cursor), 'base64').toString('utf8')); } catch (e) {}
  if (cursor && cursor.c && cursor.i) { cond.push("(createdAt < ? OR (createdAt = ? AND id < ?))"); par.push(cursor.c, cursor.c, cursor.i); }
  const rows = await allRows("SELECT * FROM leads WHERE " + cond.join(" AND ") + " ORDER BY createdAt DESC, id DESC LIMIT ?", [...par, lim + 1]);
  const temMais = rows.length > lim;
  const pagina = rows.slice(0, lim);
  const dados = pagina.map(l => v1Lead(l, req.v1.pii));
  const ult = pagina[pagina.length - 1];
  const prox = temMais && ult ? Buffer.from(JSON.stringify({ c: ult.createdAt, i: ult.id })).toString('base64') : null;
  v1Log(req, dados.length, 200);
  v1Responde(req, res, dados, { paginacao: { proximo_cursor: prox, tem_mais: temMais }, periodo: { desde: p.desde, ate: p.ate } });
});

app.get('/api/v1/leads/:id', v1Auth('leads:ler'), async (req, res) => {
  const l = await getRow("SELECT * FROM leads WHERE id = ? AND archived = 0", [req.params.id]);
  if (!l) { v1Log(req, 0, 404); return v1Erro(res, 404, 'nao_encontrado', 'Lead não encontrado.'); }
  v1Log(req, 1, 200);
  res.json({ dados: v1Lead(l, req.v1.pii) });
});

app.get('/api/v1/conversoes', v1Auth('leads:ler'), async (req, res) => {
  const p = v1Periodo(req);
  if (p.erro) { v1Log(req, 0, 400); return v1Erro(res, 400, p.erro[0], p.erro[1], p.erro[2]); }
  const rows = await allRows(
    "SELECT * FROM leads WHERE archived = 0 AND stage = 'convertida' AND COALESCE(source,'') <> 'Planilha Americano' AND (" +
    "(sale_date IS NOT NULL AND TRIM(sale_date) <> '' AND substr(sale_date,1,10) BETWEEN ? AND ?) OR " +
    "((sale_date IS NULL OR TRIM(sale_date) = '') AND substr(createdAt,1,10) BETWEEN ? AND ?)) ORDER BY COALESCE(sale_date, substr(createdAt,1,10)) DESC",
    [p.desde, p.ate, p.desde, p.ate]);
  const dados = rows.map(l => v1Lead(l, req.v1.pii));
  v1Log(req, dados.length, 200);
  v1Responde(req, res, dados, { totais: { conversoes: dados.length, receita: dados.reduce((a, x) => a + (Number(x.valor) || 0), 0) }, periodo: { desde: p.desde, ate: p.ate } });
});

app.get('/api/v1/metricas/diarias', v1Auth('metricas:ler'), async (req, res) => {
  const p = v1Periodo(req);
  if (p.erro) { v1Log(req, 0, 400); return v1Erro(res, 400, p.erro[0], p.erro[1], p.erro[2]); }
  const leads = await allRows("SELECT createdAt, source, tracking FROM leads WHERE archived = 0 AND substr(createdAt,1,10) BETWEEN ? AND ?", [p.desde, p.ate]);
  const vendas = await allRows(
    "SELECT sale_date, createdAt, source, value FROM leads WHERE archived = 0 AND stage = 'convertida' AND COALESCE(source,'') <> 'Planilha Americano' AND (" +
    "(sale_date IS NOT NULL AND TRIM(sale_date) <> '' AND substr(sale_date,1,10) BETWEEN ? AND ?) OR " +
    "((sale_date IS NULL OR TRIM(sale_date) = '') AND substr(createdAt,1,10) BETWEEN ? AND ?))",
    [p.desde, p.ate, p.desde, p.ate]);
  const mapa = {};
  const garante = (d) => (mapa[d] = mapa[d] || { dia: d, leads: 0, conversoes: 0, receita: 0, por_origem: {} });
  const org = (m, o) => (m.por_origem[o] = m.por_origem[o] || { leads: 0, conversoes: 0, receita: 0 });
  leads.forEach(l => { const d = String(l.createdAt || '').slice(0, 10); if (!d) return; const m = garante(d); m.leads++; org(m, l.source || 'Sem identificação').leads++; });
  vendas.forEach(l => {
    const d = (l.sale_date && String(l.sale_date).slice(0, 10)) || String(l.createdAt || '').slice(0, 10); if (!d) return;
    const m = garante(d), v = Number(l.value) || 0;
    m.conversoes++; m.receita += v;
    const o = org(m, l.source || 'Sem identificação'); o.conversoes++; o.receita += v;
  });
  const dados = Object.values(mapa).sort((a, b) => a.dia < b.dia ? -1 : 1);
  const totais = dados.reduce((a, x) => ({ leads: a.leads + x.leads, conversoes: a.conversoes + x.conversoes, receita: a.receita + x.receita }), { leads: 0, conversoes: 0, receita: 0 });
  v1Log(req, dados.length, 200);
  v1Responde(req, res, dados, { totais, periodo: { desde: p.desde, ate: p.ate } });
});

app.get('/api/v1/funil', v1Auth('metricas:ler'), async (req, res) => {
  const p = v1Periodo(req);
  if (p.erro) { v1Log(req, 0, 400); return v1Erro(res, 400, p.erro[0], p.erro[1], p.erro[2]); }
  const rows = await allRows("SELECT stage, COUNT(*) n, SUM(CASE WHEN stage='convertida' THEN value ELSE 0 END) v FROM leads WHERE archived = 0 AND substr(createdAt,1,10) BETWEEN ? AND ? GROUP BY stage", [p.desde, p.ate]);
  const dados = rows.map(r => ({ etapa: V1_ETAPAS[r.stage] || r.stage, leads: r.n, receita: Number(r.v) || 0 }));
  v1Log(req, dados.length, 200);
  v1Responde(req, res, dados, { periodo: { desde: p.desde, ate: p.ate } });
});

app.get('/api/v1/origens', v1Auth('metricas:ler'), async (req, res) => {
  const rows = await allRows("SELECT COALESCE(NULLIF(TRIM(source),''),'Sem identificação') origem, COUNT(*) leads FROM leads WHERE archived = 0 GROUP BY 1 ORDER BY 2 DESC");
  v1Log(req, rows.length, 200);
  v1Responde(req, res, rows);
});

// [API-V1] Administracao das chaves de parceiro (tela em Configuracoes). So Administrador.
function v1SoAdmin(req, res) {
  if (req.user && req.user.role === 'Vendedor') { res.status(403).json({ error: 'Sem permissão' }); return false; }
  return true;
}
app.get('/api/partner-keys', authenticateToken, async (req, res) => {
  if (!v1SoAdmin(req, res)) return;
  try {
    const rows = await allRows("SELECT id, nome, key_prefixo, escopos, ativa, criada_em, criada_por, ultimo_uso_em, revogada_em FROM api_partner_keys ORDER BY criada_em DESC");
    const uso = await allRows("SELECT key_id, COUNT(*) chamadas, SUM(registros) registros FROM api_access_log WHERE ts > ? GROUP BY key_id", [new Date(Date.now() - 30 * 86400000).toISOString()]);
    const mapa = {}; uso.forEach(u => { mapa[u.key_id] = u; });
    res.json(rows.map(r => Object.assign({}, r, { uso_30d: mapa[r.id] || { chamadas: 0, registros: 0 } })));
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/partner-keys', authenticateToken, async (req, res) => {
  if (!v1SoAdmin(req, res)) return;
  try {
    const nome = String((req.body && req.body.nome) || '').trim();
    if (!nome) return res.status(400).json({ error: 'Dê um nome ao parceiro (ex.: Agência X).' });
    const permitidos = ['leads:ler', 'metricas:ler', 'pii:ler'];
    let escopos = Array.isArray(req.body && req.body.escopos) ? req.body.escopos.filter(e => permitidos.includes(e)) : ['leads:ler', 'metricas:ler'];
    if (!escopos.length) escopos = ['metricas:ler'];
    // A chave e mostrada UMA vez. No banco fica so o hash — nem o administrador consegue ver depois.
    const chave = 'vv_' + crypto_v1.randomBytes(24).toString('hex');
    const hash = crypto_v1.createHash('sha256').update(chave).digest('hex');
    const id = 'pk_' + crypto_v1.randomBytes(6).toString('hex');
    await runQuery("INSERT INTO api_partner_keys (id, nome, key_hash, key_prefixo, escopos, ativa, criada_em, criada_por) VALUES (?,?,?,?,?,1,?,?)",
      [id, nome, hash, chave.slice(0, 10), escopos.join(','), new Date().toISOString(), (req.user && req.user.name) || '']);
    res.json({ id, nome, escopos, chave, aviso: 'Copie a chave agora — ela não será mostrada de novo.' });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/partner-keys/:id', authenticateToken, async (req, res) => {
  if (!v1SoAdmin(req, res)) return;
  try {
    await runQuery("UPDATE api_partner_keys SET ativa = 0, revogada_em = ? WHERE id = ?", [new Date().toISOString(), req.params.id]);
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.get('/api/partner-keys/:id/acessos', authenticateToken, async (req, res) => {
  if (!v1SoAdmin(req, res)) return;
  try {
    const rows = await allRows("SELECT ts, rota, parametros, registros, status, ip FROM api_access_log WHERE key_id = ? ORDER BY ts DESC LIMIT 100", [req.params.id]);
    res.json(rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get('/api/dashboard/sales-daily', authenticateToken, async (req, res) => {
  try {
    ...
    const fromIso = days[0].iso, toIso = days[days.length - 1].iso;
    const rows = await allRows(
      "SELECT sale_date, createdAt, tracking, source, source_locked, tags, value FROM leads WHERE stage = 'convertida' AND archived = 0 AND COALESCE(source,'') <> 'Planilha Americano' AND (" +
    ...
    // execucao7: byOrigin/byOriginCount = quebra pela origem DINÂMICA (lista configurada) — o
    // front novo usa esses mapas; byCat/byCatCount (4 baldes) continuam p/ retrocompat.
    const _originsCfg = await getLeadOriginsList();
    const byDay = {};
    rows.forEach(l => {
      ...
      try {
        const lb = leadOriginLabel(l, _originsCfg);
    ...
    const series = days.map(d => {
      ...
      return { day: d.iso, label: d.label, count: e.count, value: e.value, byCat: e.byCat, byCatCount: e.byCatCount, byOrigin: e.byOrigin || {}, byOriginCount: e.byOriginCount || {} };
    });
    const totals = { count: 0, value: 0, byCat: emptyCat(), byCatCount: emptyCat(), byOrigin: {}, byOriginCount: {} };
    series.forEach(d => {
      ...
      Object.keys(d.byOrigin || {}).forEach(k => { totals.byOrigin[k] = (totals.byOrigin[k] || 0) + d.byOrigin[k]; });
      Object.keys(d.byOriginCount || {}).forEach(k => { totals.byOriginCount[k] = (totals.byOriginCount[k] || 0) + d.byOriginCount[k]; });
    });
...
app.get('/api/dashboard/sales-daily-leads', authenticateToken, async (req, res) => {
  try {
    ...
    const rows = await allRows(
      "SELECT id, name, value, tags, tracking, source, source_locked FROM leads WHERE stage = 'convertida' AND archived = 0 AND COALESCE(source,'') <> 'Planilha Americano' AND (" +
      "(sale_date IS NOT NULL AND TRIM(sale_date) <> '' AND substr(sale_date,1,10) >= ? AND substr(sale_date,1,10) <= ?) OR " +
      "((sale_date IS NULL OR TRIM(sale_date) = '') AND substr(createdAt,1,10) >= ? AND substr(createdAt,1,10) <= ?))",
      [fromIso, toIso, fromIso, toIso]
    );
...
// execucao7 (pedido do Henry, 2026-07-26): ORIGEM DINÂMICA para os gráficos do dashboard.
// A lista configurável de "Origens de lead" (Configurações) passa a rotular os gráficos —
// leads com origem explícita (ex.: Indicação, Ferramenta de IA, Passou na Frente da Loja)
// aparecem com o PRÓPRIO nome em vez de caírem em "Sem identificação"/"Orgânico" genérico.
// leadChannelCat NÃO muda (retrocompat: ponto ótimo, filtros, popups antigos).
async function getLeadOriginsList() {
  try {
    const row = await getRow("SELECT value FROM app_settings WHERE key = 'lead_origins'");
    const arr = row && row.value ? JSON.parse(row.value) : null;
    if (Array.isArray(arr) && arr.length) return arr.map(x => String(x == null ? '' : x).trim()).filter(Boolean);
  } catch (e) {}
...
app.listen(PORT, async () => {
  ...
  // BOAS-VINDAS AUTOMÁTICAS: 1ª mensagem para leads sem conversa (formulário/orgânico que nunca
  // escreveram, ex.: Kauan). Antes era só manual — agora dispara sozinho (1ª passada ~4 min após o
  // boot, depois a cada 10 min), respeitando horário comercial + linha conectada + antiban.
  setTimeout(() => { welcomeAutoSweep().catch(() => {}); }, 4 * 60 * 1000);
  setInterval(() => { welcomeAutoSweep().catch(() => {}); }, 10 * 60 * 1000);
