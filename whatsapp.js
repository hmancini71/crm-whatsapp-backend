...
// fluxo de mensagens — qualquer erro só loga. Usada nos áudios RECEBIDOS e ENVIADOS.
function fireTranscricaoAudio(msgId, mediaPath) {
  try {
    if (!msgId || !mediaPath) return;
    transcreverAudio(mediaPath).then((t) => {
      if (t === null || t === undefined) return; // falha/sem chave → mantém NULL (botão de retry no CRM)
      runQuery("UPDATE messages SET transcription = ? WHERE id = ?", [t, msgId])
        .then(() => { try { bustConversationsCache(); } catch (e) {} })
        .catch((e) => console.error('[transcricao] Falha ao gravar transcrição de ' + msgId + ':', e && e.message));
    }).catch((e) => console.error('[transcricao] Erro inesperado (' + msgId + '):', e && e.message));
  ...
  sock.ev.on('messages.upsert', async (m) => {
    try {
      ...
      for (const msg of m.messages) {
        ...
        // transcricao-20260728: áudio (recebido OU eco de enviado) → transcreve em segundo plano.
        if (incomingType === 'audio' && incomingMediaPath) fireTranscricaoAudio(msgId, incomingMediaPath);
...
async function processNovoBacklog(limit) {
  ...
  for (const lead of leads) {
    ...
    try {
      ...
      // execucao1 Sprint1 (2026-07-25): kill-switch por lead — bot_ativo=0 tira o lead do backlog da IA.
      if (lead.bot_ativo === 0) { skipped.push((lead.name || lead.id) + ' — bot desativado no lead'); done++; continue; }
