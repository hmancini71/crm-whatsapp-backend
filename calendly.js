  ...
  for (const ev of events) {
    try {
      ...
      if (!lead) {
        semLead++;
        await runQuery(
          "INSERT INTO calendly_events (uuid, lead_id, start_time, status, card_date, updated_at) VALUES (?, NULL, ?, ?, ?, ?) " +
          "ON CONFLICT(uuid) DO UPDATE SET start_time = excluded.start_time, status = excluded.status, updated_at = excluded.updated_at",
          [uuid, startIso, status, when, Date.now()]
        );
      ...
      await runQuery(
        "INSERT INTO calendly_events (uuid, lead_id, start_time, status, card_date, updated_at) VALUES (?, ?, ?, ?, ?, ?) " +
        "ON CONFLICT(uuid) DO UPDATE SET lead_id = excluded.lead_id, start_time = excluded.start_time, status = excluded.status, card_date = excluded.card_date, updated_at = excluded.updated_at",
        [uuid, lead.id, startIso, status, when, Date.now()]
      );
