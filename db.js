...
db.serialize(() => {
  ...
  // execucao2 (2026-07-25): prazo de análise do relatório diário — analyzed_at (ISO) marca quando
  // o usuário abriu o relatório completo; null = ainda não analisado (card cobra dentro do prazo).
  db.all("PRAGMA table_info(ai_reports)", (arErr, arCols) => {
    if (!arErr && arCols && arCols.length && !arCols.find(c => c.name === 'analyzed_at')) {
      db.run("ALTER TABLE ai_reports ADD COLUMN analyzed_at TEXT", (alterErr) => {
        if (!alterErr) console.log("Migration: added 'analyzed_at' column to ai_reports.");
      });
    }
  });
  // execucao6 (2026-07-25): vínculo card ↔ contrato da base Vale Visto (botão "📄 Contrato" do
  // modal). Gravado automaticamente quando o contrato é localizado, ou pela conexão manual.
  db.all("PRAGMA table_info(leads)", (ctErr, ctCols) => {
    if (!ctErr && ctCols && ctCols.length && !ctCols.find(c => c.name === 'contract_id')) {
      db.run("ALTER TABLE leads ADD COLUMN contract_id TEXT", (alterErr) => {
        if (!alterErr) console.log("Migration: added 'contract_id' column to leads.");
      });
    }
  });
  ...
  db.all("PRAGMA table_info(calendly_events)", (ceErr, ceCols) => {
    ...
    if (!hasCe('reschedule_url')) db.run("ALTER TABLE calendly_events ADD COLUMN reschedule_url TEXT DEFAULT ''");
  });
  ...
  // execucao8 (pedido do Henry, 2026-07-26): contratos criados MANUALMENTE no CRM sem envio
  // para assinatura do cliente — ficam SÓ aqui (não vão ao sistema PHP Vale Visto). A guia
  // Contratos mescla esta tabela à lista do PHP (GET /api/contracts) com flag manual:1.
  db.run(`CREATE TABLE IF NOT EXISTS manual_contracts (
    id TEXT PRIMARY KEY,
    client_name TEXT,
    client_email TEXT,
    client_phone TEXT,
  ...
  // execucao11 (pedido do Henry, 2026-07-26): campos extras do contrato manual — endereço,
  // nº de solicitantes, primeiro visto/renovação, 2º e-mail e "como nos conheceu" (obrigatório).
  db.all("PRAGMA table_info(manual_contracts)", (mcErr, mcCols) => {
    if (mcErr || !mcCols || !mcCols.length) return;
    [['address', 'TEXT'], ['applicants', 'TEXT'], ['visa_type', 'TEXT'], ['email2', 'TEXT'], ['referral', 'TEXT']].forEach(cd => {
      if (!mcCols.find(c => c.name === cd[0])) {
        db.run('ALTER TABLE manual_contracts ADD COLUMN ' + cd[0] + ' ' + cd[1], (aErr) => {
          if (!aErr) console.log("Migration: added '" + cd[0] + "' to manual_contracts.");
        });
      }
    });
  });
  ...
  db.all("PRAGMA table_info(leads)", (liErr, lcols) => {
    ...
    if (!has('destino_visto')) db.run("ALTER TABLE leads ADD COLUMN destino_visto TEXT DEFAULT ''");
    if (!has('tipo_visto')) db.run("ALTER TABLE leads ADD COLUMN tipo_visto TEXT DEFAULT ''");
    if (!has('prazo_viagem')) db.run("ALTER TABLE leads ADD COLUMN prazo_viagem TEXT DEFAULT ''");
    if (!has('qtd_solicitantes')) db.run("ALTER TABLE leads ADD COLUMN qtd_solicitantes INTEGER DEFAULT NULL");
    if (!has('score')) db.run("ALTER TABLE leads ADD COLUMN score INTEGER DEFAULT NULL");
    if (!has('status_pagamento')) {
      db.run("ALTER TABLE leads ADD COLUMN status_pagamento TEXT DEFAULT 'lead'", (spErr) => {
        // Backfill único e idempotente: quem já assinou contrato vira 'contrato_ativo'.
        // (guard: só se contract_signed já existir neste banco — em produção existe)
        if (has('contract_signed')) db.run("UPDATE leads SET status_pagamento = 'contrato_ativo' WHERE contract_signed = 1 AND (status_pagamento IS NULL OR status_pagamento = 'lead')", () => {});
      });
    } else if (has('contract_signed')) {
      db.run("UPDATE leads SET status_pagamento = 'contrato_ativo' WHERE contract_signed = 1 AND (status_pagamento IS NULL OR status_pagamento = 'lead')", () => {});
    }
    // bot_ativo = 0 desliga TODA fala automática da IA para o lead (caso Wesley/Eliana).
  ...
  db.run(`CREATE TABLE IF NOT EXISTS compromissos (
    ...
    cumprido_em TEXT DEFAULT NULL,
    notas TEXT DEFAULT '',
    criado_em TEXT,
    criado_por TEXT DEFAULT ''
  )`, (ceErr) => {
    if (ceErr) { console.error("Failed to create compromissos:", ceErr); return; }
    db.run("CREATE INDEX IF NOT EXISTS idx_compromissos_lead ON compromissos(lead_id)");
    db.run("CREATE INDEX IF NOT EXISTS idx_compromissos_pendentes ON compromissos(cumprido, data_hora)");
  });
  ...
  // Safe migration: coluna 'transcription' em messages (transcricao-20260728) — texto transcrito
  // do áudio via Gemini. NULL = ainda não transcrito (frontend mostra botão Transcrever);
  // '' = transcrito porém vazio/sem fala. Preenchida de forma assíncrona (fire-and-forget).
  db.all("PRAGMA table_info(messages)", (err, cols) => {
    if (!err && cols && !cols.find(c => c.name === 'transcription')) {
      db.run("ALTER TABLE messages ADD COLUMN transcription TEXT DEFAULT NULL", (alterErr) => {
        if (alterErr) console.error("Failed to add transcription column to messages:", alterErr);
        else console.log("Migration: added 'transcription' column to messages table.");
      });
    }
  });
